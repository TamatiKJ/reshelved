...
const PENDING_SIGNUP_EXPIRY_MS = 60 * 60 * 1000;
...
const generateSessionId = () => crypto?.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
const getInitialOnboardingStatus = (user: User): UserProfile['onboardingStatus'] => user.emailVerified ? 'password_required' : 'pending';
...
const buildUserProfile = (user: User, displayName?: string, location = '', isAdmin = false, onboardingStatus?: UserProfile['onboardingStatus']): UserProfile => ({
  ...
  online: onboardingStatus === 'complete',
  lastSeen: Date.now(),
  deactivated: false
});

const isPendingSignupExpired = async (sessionId: string) => {
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const syncPublicProfile = async (profile: UserProfile) => {
    ...
    const ratings = ratingSnap ? ratingSnap.docs.map((item) => Number(item.data().rating || 0)).filter((rating) => rating > 0) : [];
    const ratingCount = ratings.length;
    const ratingAverage = ratingCount ? ratings.reduce((sum, rating) => sum + rating, 0) / ratingCount : 0;
  ...
  const updatePresence = async (uid: string, online: boolean) => {
    await setDoc(doc(db, 'users', uid), { online, lastSeen: Date.now() }, { merge: true });
  };
  ...
  const ensureUserProfile = async (user: User, displayName?: string, location = '', statusOverride?: UserProfile['onboardingStatus']) => {
    ...
    const shouldCompleteExistingPasswordUser = !statusOverride && user.emailVerified && !hasPendingEmailSignup;
    const nextStatus = statusOverride || (shouldCompleteExistingPasswordUser ? 'complete' : undefined);

    if (snap.exists()) {
      const existingProfile = snap.data() as UserProfile;

      if (existingProfile.onboardingStatus === 'password_required' && !statusOverride) {
      ...
      const normalizedProfile: UserProfile = {
        ...
        isAdmin: adminStatus,
        onboardingStatus: nextStatus || existingProfile.onboardingStatus || getInitialOnboardingStatus(user),
        online: true,
        lastSeen: Date.now(),
  ...
  const fetchProfile = async (uid: string) => {
    try {
      if (auth.currentUser?.uid === uid) await auth.currentUser.reload().catch(() => undefined);
      ...
      if (snap.exists()) {
        ...
        const normalizedProfile: UserProfile = {
          ...
          emailVerified: profile.emailVerified === true || Boolean(auth.currentUser?.emailVerified),
          location: profile.location || '',
        ...
        if (auth.currentUser?.uid === uid && (auth.currentUser.emailVerified || normalizedProfile.onboardingStatus === 'complete')) {
          syncPublicProfile(normalizedProfile).catch((err) => console.error('Public profile sync failed:', err));
          syncConversationProfile(normalizedProfile).catch((err) => console.error('Conversation avatar sync failed:', err));
  ...
  const completeEmailLinkSignIn = async () => {
    ...
    if (displayName && cred.user.displayName !== displayName) await updateProfile(cred.user, { displayName });
    await cred.user.reload().catch(() => undefined);
  ...
  const completeAuthenticatedSession = async (user: User | null) => {
    ...
    try {
      ...
      if (message.includes('banned') || message === 'SESSION_EXPIRED') {
        setCurrentUser(null);
        setUserProfile(null);
        ...
        const adminStatus = await getIsAdminFromClaims(user, true);
        setCurrentUser(user);
        setUserProfile(buildUserProfile(user, undefined, '', adminStatus));
      }
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    completeEmailLinkSignIn()
      .then((completedEmailLink) => {
        if (completedEmailLink) {
          setLoading(false);
          return;
        }
        unsubscribe = onAuthStateChanged(auth, completeIfMounted);
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => updatePresence(currentUser.uid, true).catch((err) => console.error('Error updating presence:', err)), 60 * 1000);

    const handleBeforeUnload = () => {
      setDoc(doc(db, 'users', currentUser.uid), { online: false, lastSeen: Date.now() }, { merge: true }).catch(() => undefined);
    };
  ...
  return (
    <AuthContext.Provider value={{ currentUser, userProfile, loading, register, login, sendVerificationEmail, refreshAuthUser, setAccountPassword, resetPassword, logout, refreshProfile }}>
      {children}
