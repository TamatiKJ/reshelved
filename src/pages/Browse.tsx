...
const filterLabelClass = 'mb-2 block text-xs font-bold uppercase tracking-[0.16em] text-stone-500';
...
const Browse: React.FC = () => {
  ...
  const filtered = listings.filter((l) => {
    ...
    if (search.trim()) {
      ...
      if (searchScope === 'book') return title.includes(s) || author.includes(s);
  ...
  const filterPanel = (
    <aside className={`${showFilters ? 'block' : 'hidden'} lg:block lg:sticky lg:top-24 lg:self-start rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-5`}>
      <div className="flex items-center justify-between gap-3 border-b border-stone-100 pb-4">
        ...
        <button type="button" onClick={clearFilters} className="cursor-pointer text-sm font-bold text-[#1665CC] hover:underline">Clear</button>
      </div>
      ...
      <div className="mt-5 space-y-5">
        ...
        <div>
          ...
          <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className={selectClass}>
            ...
            {listingCategories.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
          </select>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="w-full lg:w-2/3">
              <h1 className="w-full text-3xl font-bold text-stone-950 sm:text-5xl">Find free & affordable books near you</h1>
              <p className="mt-4 text-lg text-stone-600">Search by title, author, genre, condition, and location.</p>
            </div>
            {currentUser ? (
              <Link to="/create" className="inline-flex cursor-pointer items-center justify-center rounded-xl bg-primary-600 px-5 py-3 font-semibold text-white transition hover:bg-primary-700">List a Book</Link>
            ) : (
              <Link to="/register" className="inline-flex items-center justify-center rounded-xl bg-primary-600 px-5 py-3 font-semibold text-white transition hover:bg-primary-700">Join Reshelved</Link>
            )}
          </div>
        </div>
      </section>

      <section id="browse-results" className="mx-auto max-w-7xl scroll-mt-24 px-4 py-8 sm:px-6">
        <div className="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <div className="min-w-0">
            <div className="mb-5 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-5">
              <div className="flex flex-col gap-3 sm:flex-row">
                <div className="relative flex-1">
                  ...
                  <input type="text" placeholder="Search title, author, genre, field, condition, or location..." value={search} onChange={(e) => { setSearch(e.target.value); setSearchScope('all'); }} className={`w-full rounded-xl border border-stone-200 py-3 pl-10 pr-4 text-sm transition ${focusFieldClass}`} />
                </div>
                <button type="button" onClick={() => setShowFilters((current) => !current)} className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-stone-200 px-5 py-3 text-sm font-semibold text-stone-700 transition hover:border-[#1665CC] hover:bg-[#1665CC]/5 hover:text-[#1665CC] lg:hidden">
            ...
            {loading ? (
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">{[...Array(12)].map((_, i) => <div key={i} className="animate-pulse overflow-hidden rounded-2xl border border-stone-200 bg-white"><div className="aspect-[4/3] bg-stone-200" /><div className="space-y-3 p-4"><div className="h-4 w-3/4 rounded bg-stone-200" /><div className="h-3 w-1/2 rounded bg-stone-100" /></div></div>)}</div>
            ) : filtered.length === 0 ? (
              <div className="rounded-3xl border border-stone-200 bg-white py-16 text-center"><i className="las la-book-open text-6xl text-stone-300" /><h3 className="mt-3 text-lg font-bold text-stone-800">No books found</h3><p className="mt-1 text-stone-500">Try adjusting your filters or search terms.</p>{currentUser && <Link to="/create" className="mt-4 inline-block rounded-xl bg-primary-600 px-5 py-2.5 font-semibold text-white transition hover:bg-primary-700">List the first book</Link>}</div>
            ) : (
              <>
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">{paginatedListings.map((listing) => <BookCard key={listing.id} listing={listing} />)}</div>
                {totalPages > 1 && (
