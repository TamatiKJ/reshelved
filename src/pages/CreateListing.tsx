...
const focusFieldClass =
  "focus:border-[#1665CC] focus:ring-2 focus:ring-[#1665CC]/10";
...
const listingTypes: Array<{
...
}> = [
  {
    ...
    label: "Swap",
    icon: "las la-sync",
    desc: "Trade for another book",
  },
  {
    value: "donate",
    label: "Donate",
    icon: "las la-gift",
    desc: "Give away for free",
  },
...
const loadImage = (src: string): Promise<HTMLImageElement> =>
  new Promise((resolve, reject) => {
    ...
    image.onerror = () =>
      reject(
        new Error("Could not read this image. Try a JPG, PNG, or WebP file."),
      );
    image.src = src;
  });
...
const CreateListing: React.FC = () => {
  ...
  const activeListingType = useMemo(
    () => listingTypes.find((item) => item.value === type),
    [type],
  );
  ...
  const safeListingDays = useMemo(
    () => clampListingDays(listingDays),
    [listingDays],
  );
  ...
  const previewLocation = location || "Location";
  const previewCategory = selectedCategory?.name || category || "Category";

  useEffect(() => {
    const loadListingDays = async () => {
      const snapshot = await getDoc(doc(db, "platform", "settings")).catch(
        () => null,
      );
      const days = snapshot?.exists()
        ? snapshot.data().listingDays
  ...
  const validateStepOne = () => {
    if (images.length < 1)
      return "Upload at least one book photo before continuing.";
    return "";
  };
  ...
  const validateStepTwo = () => {
    ...
    if (!description.trim())
      return "Add a short description before continuing.";
    ...
    if (type === "sell" && (!price.trim() || getValidSellPrice(price) <= 0))
      return "Add a valid price before continuing.";
    return "";
  };
  ...
  const handleNextStep = () => {
    ...
    goToStep(nextStep as CreateStep);
  };
  ...
  const uploadSingleFile = async (
    ...
    const storageRef = ref(
      storage,
      `listings/${currentUser.uid}/${listingId}_${Date.now()}_${safeFileName}.webp`,
    );
    const task = uploadBytesResumable(storageRef, compressedBlob, {
    ...
    return new Promise((resolve, reject) => {
      ...
      const stallTimer = window.setTimeout(() => {
        if (!movedBytes) {
          ...
          reject(
            new Error(
              "Image upload did not start. Check your internet connection and Firebase Storage rules.",
            ),
          );
        }
      ...
      task.on(
        ...
        (snapshot) => {
          ...
          setUploadProgress({
            ...
            phase:
              snapshot.state === "paused" ? "Upload paused" : "Uploading image",
            fileName: file.name,
            currentFile: index + 1,
            totalFiles,
  ...
  const uploadListingImages = async (listingId: string, files: File[]) => {
    ...
    for (let i = 0; i < files.length; i += 1) {
      imageUrls.push(
        await uploadSingleFile(listingId, files[i], i, files.length),
      );
    }
    return imageUrls;
  };
  ...
  const syncDefaultLocationIfNeeded = async () => {
    if (!currentUser || !userProfile || !location) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let publishStage = "preparing listing";
    ...
    try {
      ...
      if (!categoryName)
        throw new Error("Choose a valid category before publishing.");
      if (type === "sell" && finalPrice <= 0)
        throw new Error("Add a valid price before publishing.");
      ...
      if (imageUrls.length < 1 || imageUrls.length > MAX_IMAGES)
        throw new Error("Upload between 1 and 4 listing images.");
      ...
      const listingData = {
        ...
        categoryId:
          selectedCategory?.id || selectedCategory?.slug || categoryName,
        categoryName,
        type,
        price: finalPrice,
        images: imageUrls,
      ...
      await syncDefaultLocationIfNeeded().catch((profileSyncError) => {
        console.warn(
          "Listing published, but profile location sync failed:",
          profileSyncError,
        );
      });
      ...
      window.setTimeout(() => navigate(`/listing/${listingRef.id}`), 900);
    } catch (err: any) {
      console.error(`Failed while ${publishStage}:`, err);
      setError(
        `Failed while ${publishStage}. ${err?.message || "Check your Firebase rules."}`,
      );
  ...
  const cancelButton = (
    <button
      ...
      disabled={loading}
      className="ml-auto cursor-pointer px-2 py-3 text-sm font-bold text-red-600 transition hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-50"
    >
      Cancel and Exit
    </button>
  );
      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-5">
        ...
        <div className="mt-4 aspect-[4/3] overflow-hidden rounded-2xl bg-stone-100">
          {previews[0] ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-stone-100 text-center text-sm font-semibold text-stone-400">
              Cover image will appear here
            </div>
          )}
        </div>
        <p className="body-text mt-4 font-bold text-stone-950">
          {previewTitle}
        </p>
        ...
        <div className="mt-5 grid grid-cols-3 gap-2 text-xs font-bold text-stone-600">
          <span className="min-w-0 truncate">
            <i className="las la-check-circle mr-1 text-primary-600" />
            {previewCondition}
          </span>
          <span className="min-w-0 truncate">
            <i className="las la-map-marker mr-1 text-primary-600" />
            {previewLocation}
          </span>
          <span className="min-w-0 truncate">
            <i className="las la-book mr-1 text-primary-600" />
            {previewCategory}
          </span>
        </div>
        <div className="mt-5">
          <p className="text-sm font-bold text-stone-950">Listing Type</p>
          <span className="mt-3 inline-flex items-center gap-2 rounded-full border border-stone-200 px-4 py-2 text-sm font-bold text-stone-950">
            <i
              className={`${activeListingType?.icon || "las la-tag"} text-primary-600`}
            />
            {activeListingType?.label || "Listing"}
          </span>
        ...
        <div className="mt-5 rounded-2xl bg-green-50 p-4 text-sm leading-6 text-green-800">
          <i className="las la-info-circle mr-1 text-lg text-green-700" />
          Your listing will be active for {safeListingDays}{" "}
          {safeListingDays === 1 ? "day" : "days"} after publishing.
        </div>
      </div>
    </aside>
  );
      ...
      <div className="mx-auto max-w-[1180px]">
        <div className="mb-6 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-base leading-7 text-stone-500">
              Create a clean trustworthy listing in a few guided steps.
            </p>
          </div>
        ...
        >
          <div className="min-w-0 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm">
            <div className="grid grid-cols-3 gap-2 bg-white p-3 sm:gap-3 sm:p-4">
              {steps.map((item) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold ${active ? "bg-primary-600 text-white" : "border border-stone-300 bg-stone-100 text-stone-600"}`}
                    >
                      {item.id}
                    </span>
                    <span
                      className={`max-w-full truncate text-xs font-bold sm:text-sm ${active ? "text-stone-950" : "text-stone-500"}`}
                    >
                      {item.label}
                    </span>
                  </button>
            ...
            {error && (
              <div className="mx-4 mb-4 rounded-2xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">
                {error}
              </div>
            )}
              ...
              {step === 1 && (
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-stone-500">
                    Add up to 4 photos. The first photo becomes the cover.
                  </p>
                  <div className="mt-6 grid gap-3 sm:grid-cols-[240px_1fr]">
                    <label
                      className={`flex min-h-[210px] flex-col items-center justify-center rounded-2xl border-2 border-dashed border-stone-200 bg-[#FAFAF9] text-center transition hover:border-primary-600 hover:bg-primary-50/40 ${loading || previews.length >= MAX_IMAGES ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`}
                    >
                      <i className="las la-plus text-6xl text-primary-600" />
                      <span className="mt-2 text-base font-bold text-stone-950">
                        Drag photos here
                      </span>
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {previews.map((preview, i) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            disabled={loading}
                            className="absolute right-2 top-2 flex h-7 w-7 cursor-pointer items-center justify-center rounded-full bg-red-500 text-sm font-bold text-white opacity-0 transition group-hover:opacity-100 disabled:opacity-40"
                          >
                            ×
                          </button>
                        </div>
                      ...
                      {Array.from({
                        ...
                        <div
                          key={`empty-${i}`}
                          className="aspect-square rounded-2xl border border-stone-200 bg-white"
                        />
                      ))}
                    </div>
                  </div>
                  <p className="mt-3 text-xs leading-5 text-stone-500">
                    Upload JPG, PNG, or WebP images up to 5MB each.
                  </p>
                  ...
                  <div className="mt-5 grid gap-3 sm:grid-cols-3">
                    {listingTypes.map((item) => (
                      <button
                        ...
                        className={`cursor-pointer rounded-2xl border p-4 text-left transition disabled:cursor-not-allowed disabled:opacity-60 ${type === item.value ? "border-primary-600 bg-primary-50/60 ring-1 ring-primary-600/10" : "border-stone-200 bg-white hover:border-primary-600"}`}
                      >
                        <i className={`${item.icon} text-3xl text-stone-950`} />
                        <div className="mt-3 text-sm font-bold text-stone-950">
                          {item.label}
                        </div>
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-3">
                    <button
                      ...
                      disabled={loading}
                      className="cursor-pointer rounded-lg bg-primary-600 px-8 py-3 text-sm font-bold text-white transition hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      Next Step
                    </button>
                    {cancelButton}
                ...
                <div>
                  ...
                  <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <textarea
                        ...
                        placeholder="Tell us about the book condition, edition, and notes."
                      />
                    </div>
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setCondition(
                            e.target.value as Listing["condition"] | "",
                          )
                        }
                        disabled={loading}
                        required
                    ...
                    {!userProfile?.location && (
                      <p className="sm:col-span-2 text-xs leading-5 text-stone-500">
                        <i className="las la-info-circle mr-1.5 align-[-2px] text-base text-stone-400" />
                        <span>
                          You can change your default location in{" "}
                          <Link
                            to="/profile#settings"
                            className="font-semibold text-[#1665CC] underline underline-offset-2 hover:text-[#1254a9]"
                          >
                            profile settings
                          </Link>
                          .
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-3">
                    <button
                      ...
                      onClick={() => goToStep(1)}
                      disabled={loading}
                      className="cursor-pointer rounded-lg border border-stone-200 px-8 py-3 text-sm font-bold text-stone-600 transition hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      Back
                    </button>
                    <button
                ...
                <div>
                  ...
                  <div className="mt-6 rounded-2xl border border-green-200 bg-green-50 p-5 text-sm leading-6 text-green-800">
                    <p className="font-bold text-green-900">
                      Your listing will be active after publishing.
                    </p>
                    <p className="mt-1">
                      It will be visible on your account page, visible publicly
                      in browse results, and active for {safeListingDays}{" "}
                      {safeListingDays === 1 ? "day" : "days"}.
                    </p>
                  </div>
                  <div className="mt-6 rounded-2xl border border-stone-200 bg-stone-50 p-5">
                    <div className="grid gap-4 text-sm sm:grid-cols-2">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-stone-700">{previewAuthor}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        <span className="font-bold text-stone-500">Value</span>
                        <p className="mt-1 text-stone-700">{previewPrice}</p>
                      </div>
                    </div>
                  </div>
                  <div className="mt-6 flex flex-wrap items-center gap-3">
