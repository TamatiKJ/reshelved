...
const listingTypes: Array<{ value: ListingType; label: string; icon: string; desc: string }> = [
...
const CreateListing: React.FC = () => {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = selectedFiles.filter((file) => file.type.startsWith('image/') && file.size < MAX_IMAGE_BYTES);
  ...
  const uploadListingImages = async (listingId: string, files: File[]) => {
    ...
    for (let i = 0; i < files.length; i += 1) {
      imageUrls.push(await uploadSingleFile(listingId, files[i], i, files.length));
    }
    return imageUrls;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await syncDefaultLocationIfNeeded().catch((profileSyncError) => {
        console.warn('Listing published, but profile location sync failed:', profileSyncError);
      });
      ...
      setError(`Failed while ${publishStage}. ${err?.message || 'Check your Firebase rules.'}`);
  ...
  const cancelButton = (
    <button type="button" onClick={() => navigate('/browse')} disabled={loading} className="ml-auto cursor-pointer px-2 py-3 text-sm font-bold text-red-600 transition hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-50">
      Cancel and Exit
    </button>
  );
          ...
          <div className="min-w-0 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm">
            <div className="grid grid-cols-3 gap-2 bg-white p-3 sm:gap-3 sm:p-4">
              {steps.map((item) => {
                ...
                return (
                  <button key={item.id} type="button" onClick={() => goToStep(item.id)} disabled={loading || locked} className={`flex min-w-0 cursor-pointer flex-col items-center justify-center gap-1 rounded-xl px-2 py-3 text-center transition disabled:cursor-not-allowed disabled:opacity-50 sm:flex-row sm:justify-start sm:gap-3 sm:px-4 sm:text-left ${active ? 'bg-stone-50' : 'hover:bg-stone-50'}`}>
                ...
                <div>
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-[240px_1fr]">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {previews.map((preview, i) => <div key={preview} className="group relative aspect-square overflow-hidden rounded-2xl border border-stone-200 bg-stone-100"><img src={preview} alt="" className="h-full w-full object-cover" /><button type="button" onClick={() => removeImage(i)} disabled={loading} className="absolute right-2 top-2 flex h-7 w-7 cursor-pointer items-center justify-center rounded-full bg-red-500 text-sm font-bold text-white opacity-0 transition group-hover:opacity-100 disabled:opacity-40">×</button></div>)}
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-3"><button type="button" onClick={handleNextStep} disabled={loading} className="cursor-pointer rounded-lg bg-primary-600 px-8 py-3 text-sm font-bold text-white transition hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50">Next Step</button>{cancelButton}</div>
                </div>
              )}
              ...
              {step === 2 && (
                <div>
                  ...
                  <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
                    ...
                    <div className="sm:col-span-2"><label className={labelClass}>Description *</label><textarea required value={description} onChange={(e) => setDescription(e.target.value)} disabled={loading} rows={4} className={`${fieldClass} resize-none`} placeholder="Tell us about the book condition, edition, and notes." /></div>
                    <div><label className={labelClass}>Condition *</label><select value={condition} onChange={(e) => setCondition(e.target.value as Listing['condition'] | '')} disabled={loading} required className={`${fieldClass} pr-10`}><option value="" disabled>Select condition</option>{CONDITIONS.map((item) => <option key={item} value={item}>{item}</option>)}</select></div>
                    <div><label className={labelClass}>Category *</label><select value={category} onChange={(e) => setCategory(e.target.value)} disabled={loading} required className={`${fieldClass} pr-10`}><option value="" disabled>Select category</option>{listingCategories.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</select></div>
                    <div><label className={labelClass}>Location *</label><LocationCombobox value={location} onChange={setLocation} disabled={loading} required className={`${fieldClass} pr-11`} placeholder="Search location" /></div>
                    {type === 'sell' && <div><label className={labelClass}>Price (KSh) *</label><input type="number" required min="1" value={price} onChange={(e) => setPrice(e.target.value)} disabled={loading} className={fieldClass} placeholder="e.g. 500" /></div>}
