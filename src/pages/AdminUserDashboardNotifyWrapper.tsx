...
const AdminUserDashboardNotifyWrapper: React.FC = () => {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      snap.forEach((item) =>
        list.push({ uid: item.id, ...item.data() } as UserProfile),
      );
      list.sort((a, b) =>
  ...
  const deleteDocsWhere = useCallback(
    async (
      ...
      await Promise.all(
        snap.docs.map((item) =>
          deleteDoc(doc(db, collectionName, item.id)).catch(() => undefined),
        ),
      );
    },
    [],
  ...
  const deleteUserConversations = useCallback(async (userId: string) => {
    ...
    await Promise.all(
      conversationSnap.docs.map(async (conversation) => {
        ...
        if (messageSnap)
          await Promise.all(
            messageSnap.docs.map((messageDoc) =>
              deleteDoc(doc(db, "messages", messageDoc.id)).catch(
                () => undefined,
              ),
            ),
          );
        await deleteDoc(doc(db, "conversations", conversation.id)).catch(
          () => undefined,
        );
      }),
    );
  ...
  const deleteUserData = useCallback(
    async (targetUser: UserProfile) => {
      ...
      const confirmed = window.confirm(
        `Delete ${targetUser.displayName || targetUser.email || "this user"} and their app data? This removes their Firestore data, listings, messages, reports, ratings, notifications, and public profile. Firebase Auth account deletion still needs a Cloud Function or Admin SDK.`,
      );
      if (!confirmed) return;
      ...
      try {
        ...
        window.alert(
          "User app data deleted. Firebase Auth account deletion still needs backend admin code.",
        );
      ...
      try {
        ...
        const listingDays = Math.max(
          ...
          Math.min(
            45,
            Number(
              settingsSnap?.exists() ? settingsSnap.data().listingDays : 10,
            ) || 10,
          ),
        );
        const now = Date.now();
  ...
  useEffect(() => {
    const handleClick = (event: Event) => {
      const button = (event.target as HTMLElement | null)?.closest?.(
        ".admin-extra-action-primary",
      ) as HTMLButtonElement | null;
      if (!button || !button.textContent?.toLowerCase().includes("send update"))
        return;
  ...
  useEffect(() => {
    const applyListingFilterOrder = () => {
      const listingPanel = Array.from(
        document.querySelectorAll<HTMLElement>(".admin-tiktok-shell section"),
      ).find(
        (section) =>
          section.querySelector("h3")?.textContent?.trim() === "Listings",
      );
      const filterRow = listingPanel?.querySelector<HTMLElement>(
        ".mb-4.flex.flex-wrap.items-center.gap-2",
      );
      if (!filterRow) return;

      const buttons = Array.from(
        filterRow.querySelectorAll<HTMLButtonElement>("button"),
      );
      const allButton = buttons.find((button) =>
        button.textContent?.trim().toLowerCase().startsWith("all"),
      );
      const activeButton = buttons.find((button) =>
        button.textContent?.trim().toLowerCase().startsWith("active"),
      );
      const inactiveButton = buttons.find((button) =>
        button.textContent?.trim().toLowerCase().startsWith("inactive"),
      );
      ...
      if (
        filterRow.children[0] !== allButton ||
        filterRow.children[1] !== activeButton ||
        filterRow.children[2] !== inactiveButton
      ) {
    ...
    const timers = [0, 250, 800, 1600].map((delay) =>
      window.setTimeout(applyListingFilterOrder, delay),
    );
  ...
  useEffect(() => {
    const hydrateAdminActions = () => {
      ...
      const usersPanel = panels.find(
        (section) =>
          section.querySelector("h3")?.textContent?.trim() === "Users",
      );
      usersPanel
        ...
        .forEach((row) => {
          ...
          if (
            !email ||
            !actionsCell ||
            actionsCell.querySelector(`.${DELETE_USER_BUTTON_CLASS}`)
          )
            return;
      ...
      const listingsPanel = panels.find(
        (section) =>
          section.querySelector("h3")?.textContent?.trim() === "Listings",
      );
      listingsPanel
        ...
        .forEach((row) => {
          ...
          const listingLink = row.querySelector<HTMLAnchorElement>(
            'a[href^="/listing/"]',
          );
          const statusText = cells[7]?.textContent?.toLowerCase() || "";
          const actionsCell = cells[cells.length - 1];
          const listingId = listingLink
            ?.getAttribute("href")
            ?.split("/listing/")[1]
            ?.split(/[/?#]/)[0];
          const listingTitle =
            listingLink?.textContent?.trim() || "this listing";
          if (
            !listingId ||
            !actionsCell ||
            !statusText.includes("inactive") ||
            actionsCell.querySelector(`.${RESTORE_LISTING_BUTTON_CLASS}`)
          )
            return;

          const switchButton = actionsCell.querySelector<HTMLButtonElement>(
            "button.relative.h-6.w-11",
          );
    ...
    const timers = [0, 300, 900, 1800].map((delay) =>
      window.setTimeout(hydrateAdminActions, delay),
    );
    document.addEventListener("click", runSoon, true);
  ...
  const eligibleUsers = useMemo(
    () => (excludeAdmins ? users.filter((user) => !user.isAdmin) : users),
    [excludeAdmins, users],
  );
  ...
  const sendNotification = async () => {
    ...
    try {
      ...
      window.alert(
        `Update sent to ${recipientCount} ${recipientCount === 1 ? "user" : "users"}.`,
      );
      setOpen(false);
      setStep("form");
      setTarget("all");
  ...
  return (
    <>
      ...
      {restorePrompt && (
        <div
          className="send-update-backdrop"
          onClick={() => !restoring && setRestorePrompt(null)}
        >
          <section
            className="send-update-card restore-listing-card"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="send-update-body">
              <div className="restore-listing-icon">
                <i className="las la-redo-alt" />
              </div>
              <h5>Restore this listing?</h5>
              <p>
                {restorePrompt.title} is inactive. Restoring it will turn the
                switch back on and give it a fresh expiry date using the current
                listing duration setting.
              </p>
              <div className="send-update-actions">
                <button
                  ...
                  disabled={restoring}
                  className="primary"
                  onClick={() => restoreListing(restorePrompt.listingId)}
                >
      ...
      {open && (
        ...
        >
          <section
            className="send-update-card"
            onClick={(event) => event.stopPropagation()}
          >
            {step === "form" ? (
              <>
                <div className="send-update-head">
                ...
                <div className="send-update-body">
                  ...
                  <div className="send-update-targets">
                    ...
                    >
                      ...
                      <span>
                        {loadingUsers
                          ? "Loading..."
                          : `${eligibleUsers.length} recipients`}
                      </span>
                    </button>
                    <button
                      type="button"
                      className={target === "specific" ? "active" : ""}
                      onClick={() => setTarget("specific")}
                    >
                      <strong>Specific user</strong>
                      <span>
                        {selectedUser && !selectedUserExcluded
                          ? selectedUser.displayName || selectedUser.email
                          : "Search by name or email"}
                      </span>
                    </button>
                  </div>
                  <label className="send-update-check">
                    <input
                      type="checkbox"
                    ...
                    <small>
                      {excludeAdmins
                        ? `${adminCount} admin ${adminCount === 1 ? "account is" : "accounts are"} excluded by default.`
                        : "Admins will receive this update too."}
                    </small>
                  </label>
                  {target === "specific" && (
                    <div className="send-update-picker">
                      <input
                        value={search}
                      ...
                      <div>
                        {filteredUsers.map((user) => (
                          <button
                            ...
                            type="button"
                            className={
                              selectedUserId === user.uid ? "active" : ""
                            }
                  ...
                  <div className="send-update-field">
                    <label>Subject</label>
                    <input
                      value={subject}
                ...
                <div className="send-update-body">
                  <div className="send-update-summary">
                    <div>
                      ...
                      <strong>
                        {target === "all"
                          ? `All non-admin users — ${recipientCount} recipients`
                          : `${selectedUser?.displayName || selectedUser?.email || "Specific user"} — 1 recipient`}
                      </strong>
                    </div>
                    <div>
                      <span>Admins</span>
                      <strong>
                        {excludeAdmins ? `${adminCount} excluded` : "Included"}
                      </strong>
                    </div>
                    <div>
                      <span>Subject</span>
                  ...
                  <div className="send-update-warning">
                    <i className="las la-exclamation-triangle" /> This will
                    create {recipientCount} notification{" "}
                    {recipientCount === 1 ? "document" : "documents"} in
