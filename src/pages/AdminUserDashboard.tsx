...
type Tab = 'overview' | 'listings' | 'reports' | 'registeredUsers' | 'legalPages';
...
const AdminUserDashboard: React.FC = () => {
  ...
  const filteredActiveListings = activeListings.filter((listing) => {
    ...
    if (!term) return true;
    return [listing.title, listing.author, listing.userName, listing.location, listing.condition, listing.type].filter(Boolean).some((value) => value.toLowerCase().includes(term));
  });

  const filteredReports = reports.filter((report) => {
    const term = reportSearch.trim().toLowerCase();
    if (!term) return true;
    return [report.targetName, report.reporterName, report.reason, report.details, report.targetType].filter(Boolean).some((value) => value.toLowerCase().includes(term));
  });
  ...
  const filteredUsers = users
    .filter((user) => {
      ...
      if (!term) return true;
      return [user.displayName, user.email, user.location, user.uid, user.isAdmin ? 'administrator admin' : 'user'].filter(Boolean).some((value) => value.toLowerCase().includes(term));
    })
  ...
  if (!userProfile?.isAdmin) return <div className="max-w-4xl mx-auto px-4 py-16 text-center"><h2 className="text-xl font-bold text-stone-800">Access Denied</h2><p className="text-stone-500 mt-2">You do not have admin privileges.</p><Link to="/" className="mt-4 inline-block text-primary-600 font-semibold">Back to Home</Link></div>;
  ...
  const tabs: { key: Tab; label: string; count?: number }[] = [
    ...
    { key: 'legalPages', label: 'Legal Pages' }
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between mb-6">
        <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary-600">Admin Dashboard</p><h1 className="mt-2 text-3xl font-bold text-stone-900">Reshelved Platform Management</h1><p className="mt-1 text-sm text-stone-500">Manage active listings, reports, users, and footer pages.</p></div>
        <button onClick={fetchAdminData} className="inline-flex cursor-pointer items-center justify-center gap-2 px-4 py-2 border border-stone-200 text-stone-700 rounded-xl hover:bg-stone-50 transition text-sm font-semibold"><i className="las la-redo-alt text-lg" /> Refresh</button>
      </div>
      ...
      <div className="flex gap-1 bg-stone-100 rounded-xl p-1 mb-6 overflow-x-auto">
        {tabs.map((item) => <button key={item.key} onClick={() => setTab(item.key)} className={`cursor-pointer flex items-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-semibold whitespace-nowrap transition ${tab === item.key ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}>{item.label}{item.count !== undefined && <span className={`px-1.5 py-0.5 rounded-full text-xs ${tab === item.key ? 'bg-primary-100 text-primary-700' : 'bg-stone-200 text-stone-600'}`}>{item.count}</span>}</button>)}
      </div>
      {loading ? <LoadingState /> : <>
        {tab === 'overview' && <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><Panel title="Latest Active Listings" action={<button onClick={() => setTab('listings')} className="cursor-pointer text-sm font-semibold text-primary-600">View all</button>}><div className="space-y-3">{activeListings.slice(0, 6).map((listing) => <ListingPreview key={listing.id} listing={listing} />)}{activeListings.length === 0 && <EmptyState text="No active listings yet" />}</div></Panel><Panel title="Latest Reports" action={<button onClick={() => setTab('reports')} className="cursor-pointer text-sm font-semibold text-primary-600">View all</button>}><div className="space-y-3">{openReports.slice(0, 6).map((report) => <ReportPreview key={report.id} report={report} />)}{openReports.length === 0 && <EmptyState text="No open reports" />}</div></Panel><Panel title="Dashboard Summary"><SummaryGrid items={[[ 'Flagged Listings', flaggedListings.length, 'text-orange-600' ], [ 'Expired or Disabled', expiredListings.length, 'text-stone-900' ], [ 'Flagged Users', flaggedUsers.length, 'text-yellow-600' ], [ 'Online Users', onlineUsers.length, 'text-green-600' ]]} /></Panel></div>}
        {tab === 'listings' && <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden"><FilterBar search={listingSearch} setSearch={setListingSearch} placeholder="Search active listings by title, seller, location, type..." />{filteredActiveListings.length === 0 ? <EmptyState text="No active listings found" /> : <div className="divide-y divide-stone-100">{filteredActiveListings.map((listing) => <ListingRow key={listing.id} listing={listing} onToggle={toggleListing} onDelete={deleteListing} onUnflag={unflagListing} />)}</div>}</div>}
        {tab === 'reports' && <div className="space-y-4"><div className="bg-white border border-stone-200 rounded-2xl p-4"><FilterBar search={reportSearch} setSearch={setReportSearch} placeholder="Search reports by target, reporter, reason..." noBorder /></div>{filteredReports.length === 0 ? <EmptyState text="No reports found" /> : filteredReports.map((report) => <ReportRow key={report.id} report={report} onResolve={resolveReport} onReopen={reopenReport} />)}</div>}
        {tab === 'registeredUsers' && <UsersTable users={users} adminUsers={adminUsers} filteredUsers={filteredUsers} currentUserId={currentUser?.uid} userSearch={userSearch} setUserSearch={setUserSearch} userRoleFilter={userRoleFilter} setUserRoleFilter={setUserRoleFilter} />}
        {tab === 'legalPages' && <AdminLegalPagesEditor />}
      </>}
    </div>
  );
};
...
const ListingPreview: React.FC<{ listing: Listing }> = ({ listing }) => { const image = normalizeImages(listing.images)[0]; return <div className="flex items-center gap-3"><ImageBox image={image} /><div className="min-w-0"><Link to={`/listing/${listing.id}`} className="font-semibold text-stone-900 hover:text-primary-700 truncate block">{listing.title}</Link><p className="text-xs text-stone-500 truncate">{listing.userName} · {listing.location} · {getPriceLabel(listing)}</p></div></div>; };
...
const UsersTable: React.FC<{ users: UserProfile[]; adminUsers: UserProfile[]; filteredUsers: UserProfile[]; currentUserId?: string; userSearch: string; setUserSearch: (v: string) => void; userRoleFilter: UserRoleFilter; setUserRoleFilter: (v: UserRoleFilter) => void; }> = ({ users, adminUsers, filteredUsers, currentUserId, userSearch, setUserSearch, userRoleFilter, setUserRoleFilter }) => <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden"><div className="flex flex-col gap-4 border-b border-stone-200 p-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-2 text-sm"><button type="button" onClick={() => setUserRoleFilter('all')} className={`cursor-pointer font-semibold ${userRoleFilter === 'all' ? 'text-stone-900' : 'text-primary-700 hover:text-primary-800'}`}>All <span className="text-stone-400">({users.length})</span></button><span className="text-stone-300">|</span><button type="button" onClick={() => setUserRoleFilter('admin')} className={`cursor-pointer font-semibold ${userRoleFilter === 'admin' ? 'text-stone-900' : 'text-primary-700 hover:text-primary-800'}`}>Administrators <span className="text-stone-400">({adminUsers.length})</span></button></div><div className="flex flex-col gap-2 sm:flex-row sm:items-center"><input value={userSearch} onChange={(event) => setUserSearch(event.target.value)} placeholder="Search users..." className="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm outline-none focus:border-primary-400 sm:w-72" /><button type="button" className="cursor-pointer rounded-lg border border-primary-200 px-4 py-2 text-sm font-semibold text-primary-700 hover:bg-primary-50">Search Users</button></div></div><div className="overflow-x-auto"><table className="w-full min-w-[760px] border-collapse text-left text-sm"><thead className="bg-stone-50 text-stone-700"><tr className="border-b border-stone-200"><th className="px-4 py-3 font-semibold">Name</th><th className="px-4 py-3 font-semibold">Email</th><th className="px-4 py-3 font-semibold">Join Date</th><th className="px-4 py-3 font-semibold">Last Seen</th><th className="px-4 py-3 font-semibold">Role</th></tr></thead><tbody className="divide-y divide-stone-100">{filteredUsers.map((user) => <UserRow key={user.uid} user={user} currentUserId={currentUserId} />)}</tbody></table>{filteredUsers.length === 0 && <EmptyState text="No registered users found" />}</div></div>;
const UserRow: React.FC<{ user: UserProfile; currentUserId?: string }> = ({ user, currentUserId }) => <tr className={`${user.isAdmin ? 'bg-primary-50/35' : 'bg-white'} hover:bg-stone-50`}><td className="whitespace-nowrap px-4 py-3 font-semibold text-stone-900">{user.displayName || 'Unnamed user'} {user.uid === currentUserId ? <span className="text-stone-500">(You)</span> : null}</td><td className="whitespace-nowrap px-4 py-3 text-stone-600">{user.email || 'No email'}</td><td className="whitespace-nowrap px-4 py-3 text-stone-500">{formatDateTime(user.createdAt)}</td><td className="whitespace-nowrap px-4 py-3 text-stone-500">{formatDateTime(user.lastSeen)}</td><td className="whitespace-nowrap px-4 py-3"><div className="flex items-center gap-2"><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${user.isAdmin ? 'bg-primary-50 text-primary-700' : 'bg-stone-100 text-stone-600'}`}>{user.isAdmin ? 'Administrator' : 'User'}</span>{isUserOnline(user) && <span className="rounded-full bg-green-50 px-2 py-0.5 text-xs font-semibold text-green-700">Online</span>}{user.deactivated && <span className="rounded-full bg-red-50 px-2 py-0.5 text-xs font-semibold text-red-600">Banned</span>}</div></td></tr>;
