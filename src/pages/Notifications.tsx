...
const Notifications: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      q,
      (snap) => {
        const ns = mapSnapshot<Notification>(snap).sort(
          (a, b) => (b.createdAt || 0) - (a.createdAt || 0),
        );
        setNotifications(
          ns.filter((item) => !item.read && item.type !== "message"),
        );
        setLoading(false);
      },
      (err) => {
          ...
          <div className="w-full max-w-md rounded-[28px] bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div className="flex min-w-0 items-start gap-3">
                <div
                  className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${getNotificationMeta(selectedNotification.type).tone}`}
                >
                  <i
                    className={`las ${getNotificationMeta(selectedNotification.type).icon} text-2xl`}
                  />
                </div>
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-stone-500">
                    {getNotificationMeta(selectedNotification.type).label} ·{" "}
            ...
            <button
              type="button"
              onClick={() => setSelectedNotification(null)}
              className={`mt-5 w-full cursor-pointer ${primaryButtonClass}`}
            >
              Done
            </button>
          </div>
                ...
                return (
                  ...
                  >
                    <span className="flex min-w-0 items-center gap-3.5">
                      <i className={`las ${filter.icon} text-xl`} />
                      {filter.label}
                    </span>
                    <span
          ...
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-stone-500">
                Open an update to read the full message. Read updates are
                removed from this inbox.
              </p>
            </div>
            <span className="w-fit rounded-full border border-stone-200 px-3 py-1.5 text-xs font-semibold text-stone-600">
          ...
          {loading ? (
            <div className="animate-pulse rounded-[32px] border border-stone-200 bg-white p-6">
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-24 rounded-[24px] bg-stone-100" />
                ))}
              </div>
            </div>
            ...
            <EmptyState
              ...
              body="New listing updates, swaps, and system notices will appear here."
            />
          ) : (
              ...
              {filteredNotifications.map((n) => {
                ...
                return (
                  <button
                    ...
                    className="group flex w-full cursor-pointer items-center justify-between gap-4 rounded-[24px] border border-stone-200 bg-white p-4 text-left transition hover:-translate-y-0.5 hover:border-stone-300 hover:bg-stone-50 hover:shadow-sm"
                  >
                    <span className="flex min-w-0 items-start gap-3">
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="mt-0.5 block text-xs font-bold uppercase tracking-[0.14em] text-stone-400">
                          {meta.label} · {formatNotificationDate(n.createdAt)}
                        </span>
                        <span className="mt-2 block line-clamp-2 text-sm leading-6 text-stone-500">
                          {n.message}
                        </span>
                      </span>
                    </span>
