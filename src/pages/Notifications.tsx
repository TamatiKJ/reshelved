...
const Notifications: React.FC = () => {
  ...
  if (!currentUser) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center pb-10 sm:pb-[60px]">
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 pb-10 sm:pb-[60px]">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-stone-800">Notifications</h1>
          {unreadCount > 0 && <p className="text-sm text-primary-600 mt-0.5 font-medium">{unreadCount} unread</p>}
        </div>
      </div>

      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map(i => <div key={i} className="animate-pulse bg-white rounded-xl border border-stone-200 h-20" />)}
        </div>
        ...
        <div className="space-y-2">
          {notifications.map(n => (
            <div key={n.id} className="bg-white rounded-xl border border-primary-200 shadow-sm transition cursor-pointer" onClick={() => handleExpand(n.id)}>
              <div className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold truncate text-stone-800">{n.subject}</p>
