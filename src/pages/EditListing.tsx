...
const EditListing: React.FC = () => {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      ...
      try {
        ...
        setCategory(data.categoryName || data.category || ''); setType(data.type || 'swap'); setPrice(data.price ? String(data.price) : ''); setLocation(data.location || 'Lavington');
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const mergedPayload = validateListingWrite({ ...listing, title: title.trim(), author: author.trim(), description: description.trim(), condition, category: categoryName, categoryId, categoryName, type, price: type === 'sell' ? parseFloat(price) || 0 : 0, location, images: imageUrls, updatedAt: Date.now() });
      await updateDoc(doc(db, 'listings', listing.id), { title: mergedPayload.title, author: mergedPayload.author, description: mergedPayload.description, condition: mergedPayload.condition, category: categoryName, categoryId, categoryName, type: mergedPayload.type, price: mergedPayload.price || 0, location: mergedPayload.location, images: mergedPayload.images, updatedAt: Date.now() });
