...
import ListingStepper, {
  type ListingFormStep,
} from "../components/listing-form/ListingStepper";
import ListingImageCropModal from "../components/listing-form/ListingImageCropModal";
import EditListingDetailsStep from "../components/listing-form/EditListingDetailsStep";
import {
  parseListingDoc,
  validateListingWrite,
} from "../services/listingValidation";
import {
  deleteRemovedListingImages,
  normalizeListingImageUrls,
} from "../services/listingImages";
...
const fieldClass =
  "w-full rounded-2xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-950 outline-none transition disabled:bg-stone-50 focus:border-[#1665CC] focus:ring-2 focus:ring-[#1665CC]/10";
...
const cropListingImage = async (
  ...
  const baseScale = Math.min(
    IMAGE_SIZE / image.naturalWidth,
    IMAGE_SIZE / image.naturalHeight,
  );
  const drawWidth = image.naturalWidth * baseScale * zoom;
  const drawHeight = image.naturalHeight * baseScale * zoom;
  const dx =
    (IMAGE_SIZE - drawWidth) / 2 +
    (offsetX / 100) * Math.max(0, (drawWidth - IMAGE_SIZE) / 2);
  const dy =
    (IMAGE_SIZE - drawHeight) / 2 +
    (offsetY / 100) * Math.max(0, (drawHeight - IMAGE_SIZE) / 2);
...
const EditListing: React.FC = () => {
  ...
  const { categories: listingCategories, loading: categoriesLoading } =
  ...
  const validatePhotos = () =>
    editableImages.length < 1 ? "A listing needs at least one image." : "";
  ...
  const validateDetails = () => {
    ...
    if (type === "sell" && (!price.trim() || Number(price) <= 0))
      return "Add a valid price before saving.";
    return "";
  };

  const getStepError = (targetStep: ListingFormStep) =>
    targetStep >= 3
      ? validatePhotos() || validateDetails()
      : targetStep >= 2
        ? validatePhotos()
        : "";
  const canAccessStep = (targetStep: ListingFormStep) =>
    targetStep <= step || !getStepError(targetStep);
  ...
  useEffect(() => {
    const fetchListing = async () => {
      ...
      try {
        ...
        if (!data) {
          setError(
            "This listing has invalid data and cannot be edited safely.",
          );
          setListing(null);
          return;
        }
        ...
        if (!canEditListing(data)) {
          setError(
            "You can only edit your own listings. Only admins can edit all listings.",
          );
          setListing(null);
          return;
        }
        ...
        setEditableImages(
          normalizeListingImageUrls(data.images).map((url) => ({
            id: url,
            url,
          })),
        );
  ...
  const addCroppedImage = async () => {
    ...
    try {
      const croppedFile = await cropListingImage(
        cropState.file,
        cropState.src,
        cropState.zoom,
        cropState.x,
  ...
  const uploadEditedImages = async (listingId: string) => {
    ...
    return editableImages
      .map((image) => uploadedNewImages.get(image.id) || image.url)
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err?.message || "Could not save listing. Check your Firestore rules.",
      );
      setSaving(false);
      setUploadProgress({
        active: false,
  ...
  if (loading || authLoading) {
    return (
      <div className="bg-[#F7F7F5] px-4 py-8 sm:px-6 lg:py-10">
        <div className="mx-auto max-w-[1180px] rounded-2xl border border-stone-200 bg-white p-8 text-stone-500">
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center sm:px-6">
        ...
        <div className="mt-4 flex items-center justify-center gap-4">
          <Link to="/profile" className="font-semibold text-primary-600">
            Back to profile
          </Link>
          <Link to="/browse" className="font-semibold text-stone-600">
            Browse books
          </Link>
        </div>
      </div>
    ...
    <div className="bg-[#F7F7F5] px-4 py-8 sm:px-6 lg:py-10">
      <div className="mx-auto max-w-[1180px]">
        <div className="mb-6 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-base leading-7 text-stone-500">
              Update the photos and details buyers see.
            </p>
          </div>
          {listing && (
            <Link
        ...
        >
              ...
              <div className="mx-4 mb-4 rounded-xl border border-green-200 bg-green-50 p-4">
                <div className="flex items-center justify-between gap-3 text-sm">
                  <p className="truncate font-semibold text-green-700">
                    Uploading image {uploadProgress.currentFile} of{" "}
                    {uploadProgress.totalFiles}: {uploadProgress.fileName}
                  </p>
            ...
            <div className="p-4 sm:p-6 lg:p-8">
              {step === 1 && (
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-stone-500">
                    Keep, remove, or add up to 4 listing photos. New uploads use
                    the same crop editor as Create Listing.
                  </p>
                  <div className="mt-6 grid gap-3 sm:grid-cols-[240px_1fr]">
                    {canAddMoreImages && (
                      <label
                        className={`flex min-h-[210px] flex-col items-center justify-center rounded-2xl border-2 border-dashed border-stone-200 bg-[#FAFAF9] text-center transition hover:border-primary-600 hover:bg-primary-50/40 ${saving ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`}
                      >
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {editableImages.map((image) => (
                        ...
                        >
                          ...
                          {image.isNew && (
                            <span className="absolute bottom-2 left-2 rounded-full bg-primary-600 px-2 py-0.5 text-[10px] font-bold text-white">
                              New
                            </span>
                          )}
                        </div>
                      ...
                      {Array.from({
                        length: Math.max(0, MAX_IMAGES - editableImages.length),
                      }).map((_, i) => (
                        <div
                          key={`empty-${i}`}
              ...
              {step === 3 && (
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-stone-500">
                    Confirm the updated listing details before saving.
                  </p>
          ...
          <ListingPreviewCard
            ...
            emptyImageLabel="Current cover will appear here"
          />
        </form>
      </div>
      ...
      {cropState && (
        <ListingImageCropModal
          ...
          dragState={dragState}
          onDragStart={handleCropPointerDown}
          onDragMove={handleCropPointerMove}
