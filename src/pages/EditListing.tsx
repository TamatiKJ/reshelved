...
const EditListing: React.FC = () => {
  ...
  const openCropEditor = (file: File, queue: File[]) => {
    ...
    reader.onloadend = () => {
      setCropState({ file, queue, src: reader.result as string, zoom: 1, x: 0, y: 0 });
      setDragState(null);
    };
  ...
  const uploadEditedImages = async (listingId: string) => {
    ...
    for (let i = 0; i < filesToUpload.length; i += 1) {
      uploadedNewImages.set(filesToUpload[i].id, await uploadImageFile(filesToUpload[i].file!, listingId, i, filesToUpload.length));
    }
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const categoryId = selectedCategory?.id || selectedCategory?.slug || categoryName;
  ...
  if (loading || authLoading) {
    return (
      <div className="bg-[#F7F7F5] px-4 py-8 sm:px-6 lg:py-10">
        <div className="mx-auto max-w-[1180px] rounded-2xl border border-stone-200 bg-white p-8 text-stone-500">Loading listing editor...</div>
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center sm:px-6">
        ...
        <div className="mt-4 flex items-center justify-center gap-4">
          <Link to="/profile" className="font-semibold text-primary-600">Back to profile</Link>
          <Link to="/browse" className="font-semibold text-stone-600">Browse books</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-[#F7F7F5] px-4 py-8 sm:px-6 lg:py-10">
            ...
            {uploadProgress.active && (
              <div className="mx-4 mb-4 rounded-xl border border-green-200 bg-green-50 p-4">
                <div className="flex items-center justify-between gap-3 text-sm">
                  <p className="truncate font-semibold text-green-700">Uploading image {uploadProgress.currentFile} of {uploadProgress.totalFiles}: {uploadProgress.fileName}</p>
            ...
            <div className="p-4 sm:p-6 lg:p-8">
              {step === 1 && (
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-stone-500">Keep, remove, or add up to 4 listing photos. New uploads use the same crop editor as Create Listing.</p>
                  <div className="mt-6 grid gap-3 sm:grid-cols-[240px_1fr]">
                    {canAddMoreImages && (
                      <label className={`flex min-h-[210px] flex-col items-center justify-center rounded-2xl border-2 border-dashed border-stone-200 bg-[#FAFAF9] text-center transition hover:border-primary-600 hover:bg-primary-50/40 ${saving ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}>
                        <i className="las la-plus text-6xl text-primary-600" />
                        <span className="mt-2 text-base font-bold text-stone-950">Add photos</span>
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      {editableImages.map((image) => (
                        <div key={image.id} className="group relative aspect-square overflow-hidden rounded-2xl border border-stone-200 bg-stone-100">
                          <img src={image.url} alt="" className="h-full w-full object-cover" />
                          <button type="button" onClick={() => removeImage(image.id)} disabled={saving} className="absolute right-2 top-2 flex h-7 w-7 cursor-pointer items-center justify-center rounded-full bg-red-500 text-sm font-bold text-white opacity-0 transition group-hover:opacity-100 disabled:opacity-40">×</button>
                          {image.isNew && <span className="absolute bottom-2 left-2 rounded-full bg-primary-600 px-2 py-0.5 text-[10px] font-bold text-white">New</span>}
                        </div>
                      ))}
                  ...
                  <div className="mt-6">
                    <button type="button" onClick={handleNext} disabled={saving} className="cursor-pointer rounded-lg bg-primary-600 px-8 py-3 text-sm font-bold text-white transition hover:bg-primary-700 disabled:opacity-50">Next Step</button>
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="mt-6 rounded-2xl border border-stone-200 bg-stone-50 p-5">
                    <div className="grid gap-4 text-sm sm:grid-cols-2">
                      ...
                      <div><span className="font-bold text-stone-500">Listing</span><p className="mt-1 text-stone-700">{listingTypes.find((item) => item.value === type)?.label}</p></div>
                      <div><span className="font-bold text-stone-500">Value</span><p className="mt-1 text-stone-700">{previewPrice}</p></div>
                    </div>
                  </div>
                  <div className="mt-6 flex flex-wrap items-center gap-3">
                    <button type="button" onClick={() => goToStep(2)} disabled={saving} className="cursor-pointer rounded-lg border border-stone-200 px-8 py-3 text-sm font-bold text-stone-600 transition hover:bg-stone-50 disabled:opacity-50">Back</button>
                    <button type="submit" disabled={saving} className="cursor-pointer rounded-lg bg-primary-600 px-8 py-3 text-sm font-bold text-white transition hover:bg-primary-700 disabled:opacity-50">{saving ? 'Saving...' : 'Save Changes'}</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {cropState && (
        <ListingImageCropModal
          crop={{ src: cropState.src, zoom: cropState.zoom, x: cropState.x, y: cropState.y }}
          dragState={dragState}
          onDragStart={handleCropPointerDown}
          onDragMove={handleCropPointerMove}
          onDragEnd={() => setDragState(null)}
          onZoomChange={(zoom) => setCropState((current) => current ? { ...current, zoom } : current)}
          onReset={resetCrop}
          onSkip={skipCrop}
