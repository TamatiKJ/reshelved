...
const inputClass =
  "w-full rounded-md border border-stone-300 px-3 py-2.5 text-sm outline-none transition focus:border-[#1665CC] focus:ring-2 focus:ring-[#1665CC]/10";
...
const getPendingSignUpEmail = () =>
  window.localStorage.getItem(PENDING_SIGNUP_EMAIL_KEY) || "";
const getStoredSessionId = () =>
  window.localStorage.getItem(SIGNUP_SESSION_ID_KEY) || "";
...
const getAuthErrorMessage = (error: any, fallback: string) => {
  switch (error?.code) {
    ...
    case "auth/invalid-action-code":
      return "This verification link is invalid or has already been used. Request a new link.";
    case "auth/requires-recent-login":
      return "This session expired. Request a new verification link and try again.";
...
const AuthShell: React.FC<{ children: React.ReactNode; white?: boolean }> = ({
  children,
  white = false,
}) => (
  <div
    className={`min-h-screen flex flex-col ${white ? "bg-white" : "bg-stone-50"}`}
  >
  ...
  <AuthShell>
    <section className="grid min-h-[600px] w-full max-w-[1024px] overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm md:grid-cols-[1.08fr_1fr]">
      <aside className="hidden flex-col items-center justify-between bg-[#FFF4E2] px-10 py-[68px] md:flex">
        <AuthLogo />
        <p className="text-center text-base leading-7 text-stone-700">
          Find affordable books and swap with readers near you.
        </p>
      </aside>
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="absolute right-2 top-1/2 flex h-8 w-8 -translate-y-1/2 cursor-pointer items-center justify-center rounded-md text-stone-500 hover:bg-stone-100 hover:text-stone-800"
...
const SetPasswordForm: React.FC<{ sessionId?: string; compact?: boolean }> = ({
  sessionId,
  compact = false,
}) => {
  ...
  if (created) {
    return (
      <section
        className={`${compact ? "mx-auto mt-8 w-full max-w-md" : "w-full max-w-md rounded-xl border border-stone-300 bg-white px-7 py-8 shadow-sm sm:px-9"} text-center`}
      >
  ...
  return (
    <section
      className={`${compact ? "mx-auto mt-8 w-full max-w-md" : "w-full max-w-md rounded-xl border border-stone-300 bg-white px-7 py-8 shadow-sm sm:px-9"}`}
    >
      <div className="text-center">
        {!compact && <AuthLogo />}
        <h1
          className={`${compact ? "text-3xl sm:text-4xl" : "mt-7 text-xl"} font-bold text-stone-950`}
        >
          Email verified.
        </h1>
...
const EmailVerificationFlow: React.FC<{
  ...
  onChangeEmail: () => void;
}> = ({ email, sessionId, onResend, onChangeEmail }) => {
  ...
  useEffect(() => {
    ...
    return onSnapshot(
      doc(db, "pendingSignups", sessionId),
      (snapshot) =>
        setSession(
          snapshot.exists() ? (snapshot.data() as PendingSignup) : null,
        ),
      () => undefined,
    );
  ...
  const resend = async () => {
    ...
    try {
      ...
      setError(
        getAuthErrorMessage(err, "Could not resend the verification link."),
      );
    } finally {
      setResending(false);
    }
  ...
  const status =
    userProfile?.onboardingStatus || session?.onboardingStatus || "pending";
  ...
  return (
    <AuthShell white>
      <section className="w-full max-w-3xl px-4 text-center">
        <div className="mx-auto flex h-20 w-20 items-center justify-center text-primary-600">
          <i
            className={`las ${passwordRequired || completed ? "la-check-circle" : "la-envelope"} text-7xl`}
          />
        </div>
        {!passwordRequired && !completed && (
          <>
            <h1 className="mt-8 text-4xl font-black leading-tight tracking-tight text-stone-950 sm:text-6xl">
              Verify your email to create your account.
            </h1>
            <p className="mx-auto mt-8 max-w-md text-xl font-bold leading-snug text-stone-800">
              We sent a secure sign-up link to
              <br />
              {email}.
            </p>
            <p className="mt-10 text-lg font-bold" style={{ color: LINK_BLUE }}>
              Check your spam folder if the email is missing.
            </p>
            {error && <p className={errorClass}>{error}</p>}
            {message && (
              <div className="mx-auto mt-6 max-w-xl rounded-md border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-700">
                {message}
              </div>
            )}
            <button
              ...
              className="mt-10 w-full max-w-2xl cursor-pointer rounded-md border border-stone-300 bg-white px-4 py-5 text-lg font-bold text-stone-950 hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {resending
                ? "Sending..."
                : cooldownRemaining > 0
                  ? `Resend verification link in ${cooldownRemaining}s`
                  : "Resend verification link"}
            </button>
            <button
              onClick={onChangeEmail}
              className="mt-8 block w-full cursor-pointer text-lg font-bold text-stone-800 hover:underline"
            >
        ...
        {completed && (
          <>
            ...
            <Link
              to="/profile"
              className="mt-10 inline-flex w-full max-w-2xl items-center justify-center rounded-md bg-primary-600 px-4 py-5 text-lg font-bold text-white hover:bg-primary-700"
            >
              Go to profile
            </Link>
          </>
...
export const Login: React.FC = () => {
  ...
  return (
    <AuthSplitCard
      title="Welcome back"
      subtitle="Log in to continue using Reshelved."
    >
      {error && <p className={errorClass}>{error}</p>}
      {message && (
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-4">
        ...
        <div>
          ...
          <button
            ...
            className="mt-2 cursor-pointer text-sm font-semibold hover:underline disabled:cursor-not-allowed disabled:opacity-60"
...
export const VerifyEmail: React.FC = () => {
  ...
  const sessionId = useMemo(
    () => getSessionIdFromUrl() || getStoredSessionId(),
    [],
  );
  const [session, setSession] = useState<PendingSignup | null>(null);
  ...
  const email =
    session?.email || getPendingSignUpEmail() || currentUser?.email || "";
    ...
    return (
      <AuthShell white>
        <section className="text-center">
          ...
          <p className="mt-4 text-stone-600">Request a new link to continue.</p>
          <Link
            to="/register"
            className="mt-8 inline-flex rounded-md bg-primary-600 px-6 py-3 font-bold text-white"
          >
            Request new link
          </Link>
        </section>
...
export const SetPassword: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (
      !authLoading &&
      currentUser &&
      userProfile?.onboardingStatus !== "password_required" &&
      userProfile?.onboardingStatus !== "complete"
    )
      navigate("/auth/verify", { replace: true });
    if (!authLoading && userProfile?.onboardingStatus === "complete")
      navigate("/browse", { replace: true });
  }, [authLoading, currentUser, userProfile?.onboardingStatus, navigate]);

  return (
    <AuthShell>
      <SetPasswordForm sessionId={sessionId} />
    </AuthShell>
  );
};
...
export const ForgotPassword: React.FC = () => {
  ...
  const [loading, setLoading] = useState(false);
  const [cooldownRemaining, setCooldownRemaining] = useState(
    state?.sentEmail ? RESEND_COOLDOWN_SECONDS : 0,
  );
    ...
    return (
      <AuthShell>
        <section className="w-full max-w-2xl px-4 text-center">
          <div className="mx-auto flex h-32 w-32 items-center justify-center rounded-3xl border-4 border-stone-500 text-primary-600">
          ...
          <p className="mt-8 text-2xl font-bold text-stone-900">
            Don&apos;t see the email?{" "}
            <span style={{ color: LINK_BLUE }}>Check your SPAM folder.</span>
          </p>
          {error && <p className={errorClass}>{error}</p>}
          <button
            onClick={() => sendReset()}
            disabled={loading || cooldownRemaining > 0}
            className="mt-8 w-full max-w-xl cursor-pointer rounded-md border border-stone-300 bg-white px-4 py-4 text-base font-semibold text-stone-900 hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {loading
              ? "Resending..."
              : cooldownRemaining > 0
                ? `Resend email in ${cooldownRemaining}s`
                : "Resend email"}
          </button>
          <button
            onClick={() => navigate("/login")}
            className="mt-4 w-full max-w-xl cursor-pointer rounded-md border border-stone-300 bg-white px-4 py-4 text-base font-semibold text-stone-900 hover:bg-stone-50"
          >
            Go back
          </button>
        </section>
        ...
        <form onSubmit={sendReset} className="mt-7 space-y-4">
          ...
          >
            {loading
              ? "Sending..."
              : cooldownRemaining > 0
                ? `Send reset email in ${cooldownRemaining}s`
                : "Send reset email"}
          </button>
        </form>
        <button
          onClick={() => navigate("/login")}
          className="mt-4 w-full cursor-pointer rounded-md border border-stone-300 bg-white px-4 py-3 text-sm font-semibold text-stone-900 hover:bg-stone-50"
        >
          Go back
        </button>
      </section>
...
export const Register: React.FC = () => {
  ...
  useEffect(() => {
    if (!authLoading && currentUser && userProfile)
      navigate(
        userProfile.onboardingStatus === "complete"
          ? "/browse"
          : `/auth/verify${sessionId ? `?sessionId=${sessionId}` : ""}`,
        { replace: true },
      );
  ...
  const sendSignUpLink = async () => {
    ...
    setLoading(true);
    try {
      const nextSessionId = await register(
        email,
        displayName,
        "",
        sessionId || undefined,
      ...
      setError(getAuthErrorMessage(err, "Failed to send verification link."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthSplitCard
      title="Create your Reshelved account"
      subtitle="Join Reshelved and start finding books near you."
    >
      {error && <p className={errorClass}>{error}</p>}
      <form onSubmit={handleSubmit} className="mt-8 space-y-4">
        <div>
