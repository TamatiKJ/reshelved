...
const PasswordField: React.FC<{
  ...
  required?: boolean;
  placeholder?: string;
}> = ({ id, value, onChange, autoComplete, required = true, placeholder }) => {
...
export const Login: React.FC = () => {
  ...
  const handlePasswordReset = async () => {
    ...
    try {
      ...
      navigate('/forgot-password', { state: { sentEmail: email.trim().toLowerCase() } });
    } catch (err: any) {
    ...
    <AuthShell>
      <section className="w-full max-w-md rounded-xl border border-stone-300 bg-white px-7 py-8 shadow-sm sm:px-9">
        ...
        <form onSubmit={handleSubmit} className="mt-7 space-y-4">
          <div>
            <div className="mb-1 flex items-center justify-between gap-3"><label className="text-xs font-medium text-stone-800">Email</label></div>
          ...
          <div>
            <div className="mb-1 flex items-center justify-between gap-3">
              ...
              <button type="button" onClick={handlePasswordReset} disabled={resetLoading} className="cursor-pointer text-xs font-medium hover:underline disabled:cursor-not-allowed disabled:opacity-60" style={{ color: LINK_BLUE }}>
          ...
          <button type="submit" disabled={loading} className="w-full cursor-pointer rounded-md bg-primary-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50">
            {loading ? 'Logging in...' : 'Log in'}
          </button>
        </form>

        <p className="mt-6 text-center text-sm text-stone-600">
          Don&apos;t have an account? <Link to="/register" className="font-semibold hover:underline" style={{ color: LINK_BLUE }}>Sign up</Link>
        </p>
      </section>
    </AuthShell>
  );
    ...
    return (
      <AuthShell showLegal={false}>
        <section className="w-full max-w-2xl px-4 text-center">
          ...
          <button onClick={() => sendReset()} disabled={loading} className="mt-8 w-full max-w-xl cursor-pointer rounded-md border border-stone-300 bg-white px-4 py-4 text-base font-semibold text-stone-900 hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-60">
    ...
    <AuthShell>
      <section className="w-full max-w-md rounded-xl border border-stone-300 bg-white px-7 py-8 shadow-sm sm:px-9">
        ...
        <form onSubmit={sendReset} className="mt-7 space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full cursor-pointer rounded-md bg-primary-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50">{loading ? 'Sending...' : 'Send reset email'}</button>
        </form>
