      ...
      return (
        <svg {...common}>
          <path
            ...
            d="M3 12l2-2m0 0l7-7 7 7m-9 2v8m4-8v8m5-10l2 2m-2-2v10a1 1 0 01-1 1h-3m-8 0H6a1 1 0 01-1-1V10"
          />
        </svg>
      );
    case "reports":
      ...
      return (
        <svg {...common}>
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M11.05 2.93c.3-.92 1.6-.92 1.9 0l1.52 4.67a1 1 0 00.95.69h4.9c.97 0 1.37 1.24.59 1.8l-3.97 2.88a1 1 0 00-.36 1.12l1.51 4.67c.3.92-.75 1.69-1.54 1.12l-3.96-2.88a1 1 0 00-1.18 0l-3.96 2.88c-.79.57-1.84-.2-1.54-1.12l1.51-4.67a1 1 0 00-.36-1.12L3.1 10.1c-.78-.56-.38-1.8.59-1.8h4.9a1 1 0 00.95-.69l1.52-4.67z"
          />
        </svg>
      );
    case "analytics":
      return (
        <svg {...common}>
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M9 19V6m6 13V9m6 10V3M3 19v-4"
          />
        </svg>
      );
    case "activity":
      ...
      return (
        <svg {...common}>
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.59a1 1 0 01.7.29l5.42 5.42a1 1 0 01.29.7V19a2 2 0 01-2 2z"
          />
        </svg>
      );
    case "settings":
...
const Admin: React.FC = () => {
  ...
  const [listingFilter, setListingFilter] = useState<
    "all" | "active" | "expired" | "flagged"
  >("all");
  const [userSearch, setUserSearch] = useState("");
  const [userFilter, setUserFilter] = useState<
    "all" | "active" | "deactivated" | "flagged" | "admin"
  >("all");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const platformSettings = settingsDoc.exists()
  ...
  const unflagListing = async (listingId: string) => {
    ...
    const update = (l: Listing) =>
      l.id === listingId ? { ...l, flagged: false, flagCount: 0 } : l;
    setAllListings((prev) => prev.map(update));
    setFlaggedListings((prev) => prev.filter((l) => l.id !== listingId));
  ...
  const saveEditListing = async () => {
    ...
    const applyUpdates = (l: Listing) =>
      l.id === editListing.id ? { ...l, ...updates } : l;
  ...
  const toggleUserDeactivated = async (user: UserProfile) => {
    ...
    setAllUsers((prev) =>
      prev.map((u) =>
        u.uid === user.uid ? ({ ...u, deactivated } as any) : u,
      ),
    );
    setConfirmUser(null);
  ...
  const toggleAdmin = async (user: UserProfile) => {
    ...
    setAllUsers((prev) =>
      prev.map((u) => (u.uid === user.uid ? { ...u, isAdmin } : u)),
    );
    showToast(
      isAdmin
  ...
  const unflagUser = async (userId: string) => {
    ...
    const update = (u: UserProfile) =>
      u.uid === userId ? { ...u, flagged: false, flagCount: 0 } : u;
    ...
    try {
      if (notifTarget === "all") {
        ...
        const targetUser = allUsers.find(
          (u) => u.uid === notifUserId || u.email === notifUserId,
        );
        if (!targetUser) {
    ...
    () =>
      allListings.filter((l) => {
        ...
        if (listingFilter === "active")
          return matchSearch && l.active && l.expiresAt > now;
      ...
      allUsers
        .filter((u) => {
          ...
          if (userFilter === "active")
            return matchSearch && !(u as any).deactivated;
          if (userFilter === "deactivated")
            return matchSearch && (u as any).deactivated;
  ...
  if (!userProfile?.isAdmin) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-16 text-center">
        ...
        <Link to="/" className="mt-4 inline-block font-medium text-primary-600">
          Back to Home
        </Link>
      </div>
    );
  ...
  const activeListings = allListings.filter(
    (l) => l.active && l.expiresAt > Date.now(),
  );
  ...
  const NavButton = ({
    ...
    item: { key: Tab; label: string; icon: string; count?: number };
  }) => {
    ...
    return (
      <button
        ...
        className={`flex h-11 w-full items-center gap-3.5 rounded-lg px-3.5 text-left text-[16px] transition ${active ? "border border-stone-300 bg-gradient-to-b from-stone-100 to-stone-200 font-semibold text-black" : "font-medium text-stone-700 hover:bg-stone-100"}`}
      >
        <Icon name={item.icon} />
        <span className="min-w-0 flex-1 truncate">{item.label}</span>
        {item.count !== undefined && item.count > 0 && (
          <span
            className={`rounded-full px-2 py-0.5 text-xs ${active ? "bg-white text-stone-700" : "bg-stone-100 text-stone-500"}`}
          >
            {item.count}
          </span>
        )}
    ...
    <div className="min-h-screen bg-[#fafafa] text-stone-900">
      ...
      <div className="flex min-h-screen">
        <aside className="hidden min-h-screen w-[296px] shrink-0 border-r border-[#e5e5e5] bg-white px-6 pt-6 lg:block">
          ...
          >
            <span className="text-xl leading-none">+</span>Send Update
          </button>
          <nav className="mt-4">
            ...
            <div className="space-y-1.5">
              ...
              <NavButton
                item={{ key: "legal", label: "Legal Pages", icon: "legal" }}
              />
              <NavButton
            ...
            <div className="space-y-1.5">
              <Link
                to="/"
                className="flex h-11 items-center gap-3.5 rounded-lg px-3.5 text-[16px] font-medium text-stone-700 transition hover:bg-stone-100"
              >
                <Icon name="site" />
                <span>View Site</span>
              </Link>
                  ...
                  <div className="space-y-7">
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                      ...
                      <StatCard
                        ...
                        helper={`${allListings.length} total listings`}
                      />
                      <StatCard
                        label="Open Reports"
                        value={reports.length}
                        helper={`${flaggedListings.length} flagged listings`}
                      />
                      <StatCard
                        label="Flagged Users"
                    ...
                    <div className="rounded-2xl border border-stone-200 bg-white p-6">
                      ...
                      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                        {Object.entries(settings).map(([key, value]) => (
                          <div key={key} className="rounded-xl bg-stone-50 p-4">
                  ...
                  <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
                    <div className="rounded-2xl border border-stone-200 bg-white p-6">
                      ...
                      <p className="mt-1 text-sm text-stone-500">
                        Send platform announcements or account updates to users.
                      </p>
                      <div className="mt-5 space-y-4">
                        <label className="block">
                          ...
                          <select
                            value={notifTarget}
                            onChange={(e) =>
                              setNotifTarget(e.target.value as "all" | "user")
                            }
                            className="mt-1 w-full rounded-lg border border-stone-200 px-3 py-2.5 text-sm outline-none focus:border-primary-600"
                          >
                        ...
                        {notifTarget === "user" && (
                          <label className="block">
                            ...
                            <input
                              ...
                              className="mt-1 w-full rounded-lg border border-stone-200 px-3 py-2.5 text-sm outline-none focus:border-primary-600"
                              placeholder="name@email.com or UID"
                            />
                          </label>
                        )}
                        <label className="block">
                        ...
                        <button
                          ...
                          className="rounded-lg bg-primary-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          {notifSending ? "Sending..." : "Send update"}
                        </button>
                      </div>
                    </div>
                    <div className="rounded-2xl border border-stone-200 bg-white p-6">
                ...
                {tab !== "overview" && tab !== "notifications" && (
                  <div className="rounded-2xl border border-stone-200 bg-white p-6">
                    ...
                    <p className="mt-2 text-sm text-stone-500">
                      This section is available in your active admin dashboard
                      module.
                    </p>
                  </div>
                )}
