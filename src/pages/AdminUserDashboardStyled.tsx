...
const AdminUserDashboardStyled: React.FC = () => {
  ...
  const [notificationStep, setNotificationStep] =
    useState<NotificationStep>("form");
  ...
  const [selectedNotificationUserId, setSelectedNotificationUserId] =
    useState("");
  const [loadingNotificationUsers, setLoadingNotificationUsers] =
    useState(false);
  const [imageModalOpen, setImageModalOpen] = useState(false);
  const [imageModalTab, setImageModalTab] = useState<"upload" | "library">(
    "upload",
  );
  ...
  const loadNotificationUsers = useCallback(async () => {
    ...
    try {
      ...
      usersSnap.forEach((item) =>
        users.push({ uid: item.id, ...item.data() } as UserProfile),
      );
      users.sort((a, b) =>
  ...
  const selectedNotificationUser = notificationUsers.find(
    (item) => item.uid === selectedNotificationUserId,
  );
  ...
  const canReviewNotification =
    notificationSubject.trim().length > 0 &&
    notificationMessage.trim().length > 0 &&
    notificationRecipientCount > 0;
  const filteredNotificationUsers = notificationUsers
    .filter((item) => {
      const query = notificationUserSearch.trim().toLowerCase();
      if (!query) return true;
      return [item.displayName, item.email, item.uid]
        .join(" ")
        .toLowerCase()
        .includes(query);
    })
  ...
  const sendReviewedNotification = useCallback(async () => {
    ...
    try {
      ...
      window.alert(
        `Update sent to ${notificationRecipientCount} ${notificationRecipientCount === 1 ? "user" : "users"}.`,
      );
  ...
  const getEditor = useCallback(
    () =>
      document.querySelector<HTMLElement>(
        '.admin-tiktok-shell [contenteditable="true"]',
      ),
    [],
  );
  ...
  const selectionIsInsideEditor = useCallback(() => {
    ...
    return Boolean(
      ...
      activeNode &&
      editor.contains(
        activeNode.nodeType === Node.TEXT_NODE
  ...
  const syncEditorContent = useCallback(() => {
    ...
    editor?.dispatchEvent(
      new InputEvent("input", { bubbles: true, inputType: "insertHTML" }),
    );
  }, [getEditor]);
  ...
  const uploadEditorImage = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        console.error(error);
        window.alert(
          "Image upload failed. Check Firebase Storage and Firestore rules.",
        );
      } finally {
  ...
  useEffect(() => {
    ...
    const sidebarSelector =
      ".admin-tiktok-shell aside.hidden.border-r.border-stone-200.bg-white";
    const sidebars = Array.from(
      document.querySelectorAll<HTMLElement>(sidebarSelector),
    );
    ...
    sidebars.forEach((sidebar) => {
      ...
      topDivider.className =
        "admin-extra-action-divider admin-extra-action-divider-top";
      ...
      bottomDivider.className =
        "admin-extra-action-divider admin-extra-action-divider-bottom";
      ...
      logoutButton.innerHTML =
        '<i class="las la-sign-out-alt"></i><span>Sign Out</span>';

      const handleSend = () => openSendUpdateModal();
      const handleLogout = () => logout?.();

      const placeExtraActions = () => {
        if (sidebar.firstElementChild !== topWrap)
          sidebar.insertBefore(topWrap, sidebar.firstChild);
        if (sidebar.lastElementChild !== bottomWrap)
  ...
  useEffect(() => {
    ...
    const getToolbar = () =>
      getEditor()?.previousElementSibling?.querySelector<HTMLElement>(
        ".mt-4.flex.flex-wrap.gap-2",
      ) || null;
    const getToolbarButtons = () =>
      Array.from(
        getToolbar()?.querySelectorAll<HTMLButtonElement>("button") || [],
      );
    const getButtonFormat = (button: HTMLButtonElement) =>
      button.dataset.adminFormat ||
    ...
    const addParagraphButton = () => {
      ...
      if (!toolbar || toolbar.querySelector('[data-admin-format="paragraph"]'))
        return;
      ...
      paragraphButton.className =
        "admin-paragraph-button cursor-pointer rounded-lg border border-stone-200 px-3 py-1.5 text-sm font-semibold hover:bg-stone-50";
      paragraphButton.innerHTML = '<i class="las la-paragraph text-lg"></i>';

      paragraphButton.addEventListener("mousedown", (event) =>
          ...
          (event) => {
            ...
            if (isImageButton) {
              ...
              savedEditorRangeRef.current =
                savedRange?.cloneRange() || savedEditorRangeRef.current;
              openImageModal();
              return;
            }
  ...
  useEffect(() => {
    const clickAddNewPost = () => {
      const buttons = Array.from(
        document.querySelectorAll<HTMLButtonElement>(
          ".admin-tiktok-shell button",
        ),
      );
      const addNewButton = buttons.find(
        (button) => button.textContent?.trim().toLowerCase() === "add new",
      );
      addNewButton?.click();
    };
    ...
    const addPostsButton = () => {
      const headings = Array.from(
        document.querySelectorAll<HTMLHeadingElement>(
          ".admin-tiktok-shell section h3",
        ),
      );
      const postsHeading = headings.find((heading) =>
        heading.textContent?.trim().toLowerCase().startsWith("all posts"),
      );
      const panel = postsHeading?.closest("section") as HTMLElement | null;
      if (
        !postsHeading ||
        !panel ||
        panel.querySelector(".admin-add-post-button")
      )
        return;
    ...
    return () => {
      mutationObserver.disconnect();
      document
        .querySelectorAll(".admin-add-post-button")
  ...
  return (
    <div className="admin-tiktok-shell">
            ...
            {notificationStep === "form" ? (
                ...
                <div className="admin-notification-modal-body">
                  ...
                  <div className="admin-notification-target-grid">
                    ...
                    >
                      ...
                      <span>
                        {loadingNotificationUsers
                          ? "Loading recipients..."
                          : `${notificationUsers.length} recipients`}
                      </span>
                    </button>
                    <button
                      type="button"
                      className={
                        notificationTarget === "specific" ? "active" : ""
                      }
                    ...
                    >
                      ...
                      <span>
                        {selectedNotificationUser
                          ? selectedNotificationUser.displayName ||
                  ...
                  {notificationTarget === "specific" && (
                    <div className="admin-notification-user-picker">
                      <input
                        ...
                        onChange={(event) =>
                          setNotificationUserSearch(event.target.value)
                        }
                      ...
                      <div>
                        {filteredNotificationUsers.map((item) => (
                          <button
                            ...
                            type="button"
                            className={
                              selectedNotificationUserId === item.uid
                                ? "active"
                                : ""
                            }
                            onClick={() =>
                              setSelectedNotificationUserId(item.uid)
                            }
                          >
                            <strong>
                  ...
                  <label
                    className="admin-notification-label"
                    htmlFor="admin-notification-subject"
                  >
                    Subject
                  </label>
                  <input
                  ...
                  <div className="admin-notification-actions">
                    <button
                      type="button"
                      disabled={!canReviewNotification}
                      onClick={() => setNotificationStep("confirm")}
                      className="admin-notification-primary"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setNotificationModalOpen(false)}
                      className="admin-notification-secondary"
                    >
                      Cancel
                    </button>
                  </div>
              ...
              <>
                <div className="admin-notification-modal-header">
                  <button
                    type="button"
                    onClick={() => setNotificationStep("form")}
                  >
                    ← Edit notification
                  </button>
                  <span>/</span>
                ...
                <div className="admin-notification-modal-body">
                  <div className="admin-notification-summary">
                    <div>
                      ...
                      <strong>
                        ...
                        {notificationTarget === "all"
                          ? `All users — ${notificationRecipientCount} recipients`
                          : `${selectedNotificationUser?.displayName || selectedNotificationUser?.email || "Specific user"} — 1 recipient`}
                      </strong>
                    </div>
                    <div>
                      <span>Subject</span>
                      <strong>{notificationSubject.trim()}</strong>
                    </div>
                    <div>
                      <span>Message</span>
                  ...
                  <div className="admin-notification-warning">
                    <i className="las la-exclamation-triangle" /> This will
                    create {notificationRecipientCount} notification{" "}
                  ...
                  <div className="admin-notification-actions">
                    <button
                      type="button"
                      disabled={sending}
                      onClick={sendReviewedNotification}
                      className="admin-notification-primary"
                    >
                      <i className="las la-paper-plane" />{" "}
                      {sending
                        ? "Sending..."
                        : `Send to ${notificationRecipientCount} ${notificationRecipientCount === 1 ? "user" : "users"}`}
                    </button>
                    <button
                      type="button"
                      disabled={sending}
      ...
      {imageModalOpen && (
        <div
          className="admin-image-modal-backdrop"
        ...
        >
          ...
          >
            ...
            <div className="admin-image-tabs">
              <button
                type="button"
                className={imageModalTab === "upload" ? "active" : ""}
              ...
              <button
                type="button"
                className={imageModalTab === "library" ? "active" : ""}
                onClick={() => {
            ...
            {imageModalTab === "upload" ? (
              <div className="admin-image-upload-panel">
                ...
                >
                  ...
                  <span>
                    Image will be saved to Firebase Storage and inserted into
                    the post.
                  </span>
                </label>
              </div>
                ...
                {libraryImages.length > 0 ? (
                  libraryImages.map((image) => (
                    ...
                    >
                      <img
                        src={image.url}
                        alt={image.altText || image.filename || "Library image"}
                      />
