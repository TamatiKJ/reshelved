      ...
      <section className="bg-white pt-0 pb-[160px] sm:pb-[340px]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-5">
            {bookCategoryCards.map((category) => <Link key={category.title} to={category.href} className="group rounded-[22px] border border-stone-200 bg-white p-5 min-h-[230px] flex flex-col justify-between hover:-translate-y-1 hover:shadow-xl hover:border-primary-500 transition-all duration-300"><h3 className="text-[21px] leading-tight font-semibold text-stone-900">{category.title}</h3><div className="mt-8 flex items-center justify-center"><div className="relative w-28 h-28 rounded-2xl flex items-center justify-center"><img src={category.image} alt="" className="w-20 h-20 object-contain" loading="lazy" onError={(event) => { event.currentTarget.style.display = 'none'; const fallback = event.currentTarget.nextElementSibling as HTMLElement | null; if (fallback) fallback.style.opacity = '1'; }} /><span className="absolute text-5xl opacity-0 transition">{category.fallback}</span></div></div></Link>)}
          </div>
        </div>
      </section>
