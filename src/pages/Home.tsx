...
const Home: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-white pt-10 sm:pt-16 pb-[220px] sm:pb-[340px]"><div className="relative max-w-7xl mx-auto px-4 sm:px-6"><div className="text-center max-w-3xl mx-auto"><h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-stone-950">Find your next read faster</h2></div><div className="relative mt-10 mb-10 sm:mb-0 -mx-4 sm:mx-0"><div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 bg-gradient-to-l from-white/55 via-white/30 to-transparent sm:hidden" />{canScrollCategoriesLeft && <><div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-20 bg-gradient-to-r from-white/55 via-white/30 to-transparent sm:hidden" /><button type="button" onClick={() => scrollCategories('left')} aria-label="Scroll categories left" className="absolute left-4 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full bg-stone-700/95 text-white shadow-lg sm:hidden"><i className="las la-angle-left text-2xl" /></button></>}{canScrollCategoriesRight && <button type="button" onClick={() => scrollCategories('right')} aria-label="Scroll categories right" className="absolute right-4 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full bg-stone-700/95 text-white shadow-lg sm:hidden"><i className="las la-angle-right text-2xl" /></button>}<div ref={categoryScrollRef} onScroll={updateCategoryScrollButtons} className="flex gap-4 overflow-x-auto px-4 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:grid-cols-2 sm:gap-5 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-6">{bookCategoryCards.map((category) => <Link key={category.title} to={category.href} className="group w-[calc((100vw-64px)/2.5)] min-w-[calc((100vw-64px)/2.5)] rounded-[22px] border border-stone-200 bg-white p-4 min-h-[180px] flex flex-col justify-between hover:-translate-y-1 hover:shadow-xl hover:border-primary-600 transition-all duration-300 sm:w-auto sm:min-w-0 sm:p-5 sm:min-h-[190px]"><h3 className="text-[19px] sm:text-[21px] leading-tight font-semibold text-stone-900">{category.title}</h3><div className="mt-4 flex items-center justify-center"><div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl flex items-center justify-center"><span className="flex h-[60px] items-center justify-center text-[60px] leading-none" role="img" aria-label={`${category.title} category`}>{category.icon}</span></div></div></Link>)}</div></div></div></section>
      <section className="relative bg-[#121212] text-white pt-0 pb-0"><div className="max-w-5xl mx-auto px-4 sm:px-6 -translate-y-1/2 mb-[-100px] sm:mb-[-120px] relative z-10"><div className="bg-[#FFF4E2] text-stone-950 rounded-[28px] sm:rounded-[36px] px-6 sm:px-16 py-16 sm:py-24 text-center"><h2 className="text-4xl sm:text-6xl font-bold leading-tight">Don’t let your books sit unused</h2><p className="mt-5 sm:mt-8 text-stone-700">Someone needs what you already have.</p><div className="mt-5 sm:mt-6 flex flex-wrap justify-center gap-3"><Link to="/create" className="px-5 py-3 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-md transition">List a Book</Link><Link to="/browse" className="px-5 py-3 border border-stone-900 text-stone-900 font-semibold rounded-md hover:bg-stone-900 hover:text-white transition">Find Books</Link></div><p className="mt-3 text-xs text-stone-500">Quick Sign Up | It’s 100% Free!</p></div></div><div className="max-w-7xl mx-auto px-4 sm:px-6 pb-0 pt-0"><div className="text-center"><p className="text-xs font-bold tracking-[0.25em] text-white uppercase mb-4">LOVED ACROSS NAIROBI</p><h2 className="text-4xl sm:text-6xl font-bold">Join 200+ Nairobians reading more while spending less</h2></div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">{testimonials.map((review, i) => <div key={i} className="bg-white rounded-2xl border border-stone-200 p-6 flex flex-col gap-4"><div className="flex items-center gap-0.5">{[...Array(review.stars)].map((_, s) => <svg key={s} className="w-5 h-5 text-accent-500" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>)}</div><p className="text-[17px] leading-[1.4] text-black flex-1">“{review.text}”</p><div className="flex items-center gap-3 pt-2"><img src={review.image} alt={review.name} className="w-10 h-10 rounded-full object-cover bg-stone-200" loading="lazy" /><div><p className="font-semibold text-stone-800 text-sm">{review.name}</p><p className="text-[#898A88] text-xs mt-0.5">{review.location}</p></div></div></div>)}</div><div className="border-b border-white/25 pb-20 mt-28"><div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-12"><div className="text-[clamp(64px,11vw,140px)] font-bold leading-[0.92] tracking-tight text-white"><div>Start free</div><div className="inline-flex items-center gap-3 sm:gap-8"><span>today</span><Link to="/register" className="w-[clamp(58px,8vw,104px)] h-[clamp(58px,8vw,104px)] rounded-full bg-primary-600 hover:bg-primary-700 flex items-center justify-center transition shrink-0" aria-label="Join Reshelved free"><i className="las la-arrow-right text-[clamp(26px,3vw,44px)] text-white" /></Link></div></div><p className="text-white/80 text-lg max-w-md lg:pb-8">Built with feedback from readers across Nairobi. Try Reshelved and see why they love it.</p></div></div></div></section>
