...
const Home: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-white pt-10 sm:pt-16 pb-[160px] sm:pb-[340px]"><div className="max-w-7xl mx-auto px-4 sm:px-6"><div className="text-center max-w-3xl mx-auto"><h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-stone-950">Find your next read faster</h2></div><div className="mt-10 -mx-4 flex gap-5 overflow-x-auto px-4 pb-4 sm:mx-0 sm:grid sm:grid-cols-2 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-6">{bookCategoryCards.map((category) => <Link key={category.title} to={category.href} className="group min-w-[200px] rounded-[22px] border border-stone-200 bg-white p-5 min-h-[190px] flex flex-col justify-between hover:-translate-y-1 hover:shadow-xl hover:border-primary-500 transition-all duration-300 sm:min-w-0"><h3 className="text-[21px] leading-tight font-semibold text-stone-900">{category.title}</h3><div className="mt-5 flex items-center justify-center"><div className="relative w-24 h-24 rounded-2xl flex items-center justify-center"><img src={category.image} alt="" className="w-16 h-16 object-contain" loading="lazy" onError={(event) => { event.currentTarget.style.display = 'none'; const fallback = event.currentTarget.nextElementSibling as HTMLElement | null; if (fallback) fallback.style.opacity = '1'; }} /><span className="absolute text-4xl opacity-0 transition">{category.fallback}</span></div></div></Link>)}</div></div></section>
