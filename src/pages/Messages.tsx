...
const Messages: React.FC = () => {
  ...
  const getOtherParticipantInitial = (conv: Conversation) => {
    return getOtherParticipantName(conv)[0]?.toUpperCase() || 'U';
  };
              ...
              {loading ? (
                ...
                conversations.map((conv) => (
                  <Link
                    key={conv.id}
                    to={`/messages/${conv.id}`}
                    className={`block p-4 border-b border-stone-50 hover:bg-stone-50 transition ${conv.id === conversationId ? 'bg-primary-50 border-l-2 border-l-primary-500' : ''}`}
                  >
            ...
            {conversationId && selectedConv ? (
              <>
                <div className="p-4 border-b border-stone-200 flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-stone-800 text-sm">{getOtherParticipantName(selectedConv)}</h3>
                    <Link to={`/listing/${selectedConv.listingId}`} className="text-xs text-primary-600 hover:underline">
                    ...
                    return (
                      <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm ${
                          isMe ? 'bg-primary-600 text-white rounded-br-sm' : 'bg-stone-100 text-stone-800 rounded-bl-sm'
                        }`}>
                          <p>{msg.text}</p>
                ...
                <form onSubmit={handleSend} className="p-4 border-t border-stone-200">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-4 py-2.5 rounded-xl border border-stone-200 focus:border-primary-400 focus:ring-2 focus:ring-primary-100 outline-none transition text-sm"
                    />
                    <button
                      type="submit"
