...
const Profile: React.FC = () => {
  ...
  const tabs = isOwnProfile ? [
    ...
    { id: 'expired' as ProfileTab, label: 'Expired listings', short: 'Expired', icon: 'la-book-open', count: expiredListings.length },
    ...
    <div className="mx-auto max-w-[1180px] px-4 py-8 pb-10 sm:px-6 sm:pb-20">
      <div className="grid gap-8 lg:grid-cols-[320px_minmax(0,1fr)] lg:items-start">
        <aside className="lg:sticky lg:top-24">
          ...
          {isOwnProfile && <nav className="mt-5 overflow-hidden rounded-[28px] border border-stone-200 bg-white p-2 shadow-sm"><div className="grid gap-1">{tabs.map(tab => <button key={tab.id} type="button" onClick={() => { setActiveTab(tab.id); setSaveError(''); setSaveMessage(''); }} className={`flex cursor-pointer items-center justify-between rounded-2xl px-4 py-3 text-left text-sm font-bold transition ${activeTab === tab.id ? 'bg-stone-950 text-white' : 'text-stone-700 hover:bg-stone-50'}`}><span className="flex items-center gap-3"><i className={`las ${tab.icon} text-xl`} />{tab.label}</span>{tab.count !== null && <span className={`rounded-full px-2 py-0.5 text-xs ${activeTab === tab.id ? 'bg-white/15 text-white' : 'bg-stone-100 text-stone-500'}`}>{tab.count}</span>}</button>)}</div></nav>}
        </aside>
        ...
        <main className="min-w-0">
          ...
          {isOwnProfile && activeTab === 'active' && <section><SectionHeader eyebrow="Your shelf" title="Active listings" subtitle="Books buyers can currently discover and message you about." action={<Link to="/create" className="inline-flex items-center justify-center gap-2 rounded-full bg-stone-950 px-5 py-3 text-sm font-bold text-white hover:bg-stone-800"><i className="las la-plus text-lg" />List a book</Link>} />{renderListingGrid(activeListings, <EmptyState icon="la-book-open" title="No active listings" body="List your first book so people nearby can find it." action={<Link to="/create" className="inline-flex items-center justify-center rounded-full bg-stone-950 px-5 py-3 text-sm font-bold text-white hover:bg-stone-800">List your first book</Link>} />)}</section>}
