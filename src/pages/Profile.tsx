...
const Profile: React.FC = () => {
  ...
  const renderListingGrid = (items: Listing[], empty: React.ReactNode, compact = false) => items.length === 0 ? empty : <div className={compact ? 'grid grid-cols-1 gap-x-4 gap-y-5 sm:grid-cols-2 lg:grid-cols-2' : 'grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2 lg:grid-cols-3'}>{items.map(l => <BookCard key={l.id} listing={l} />)}</div>;
    ...
    <div className="mx-auto max-w-[1180px] px-4 py-8 pb-10 sm:px-6 sm:pb-20">
      ...
      <div className="grid gap-8 lg:grid-cols-[320px_minmax(0,1fr)] lg:items-start">
        ...
        <main className="min-w-0">
          {!isOwnProfile && <section><SectionHeader title={`${profile.displayName}'s books`} subtitle={`${activeListings.length} active book${activeListings.length === 1 ? '' : 's'} listed on Reshelved.`} />{renderListingGrid(activeListings.slice(0, 2), <EmptyState icon="la-book-open" title="No active listings" body="This user does not have any books available right now." />, true)}</section>}
