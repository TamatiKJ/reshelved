...
const inputClass =
  "w-full rounded-2xl border border-stone-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-stone-900 focus:ring-2 focus:ring-stone-900/10";
const primaryButtonClass =
  "inline-flex items-center justify-center gap-2 rounded-full bg-[#FF5F57] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#e84f48] disabled:cursor-not-allowed disabled:opacity-60";
const dangerButtonClass =
  "inline-flex w-full cursor-pointer items-center justify-center gap-2 rounded-full bg-red-600 px-5 py-3 text-sm font-bold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60";
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="absolute right-2 top-1/2 flex h-9 w-9 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full text-stone-500 hover:bg-stone-100"
...
const SectionHeader: React.FC<{
  ...
  <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
    <div>
      {eyebrow && (
        <p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-stone-400">
          {eyebrow}
        </p>
      )}
...
const SecurityCard: React.FC<{
  ...
  tone?: "default" | "danger";
  onClick: () => void;
}> = ({ icon, title, body, tone = "default", onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className={`group flex w-full cursor-pointer items-center justify-between gap-4 rounded-[24px] border p-4 text-left transition hover:-translate-y-0.5 hover:shadow-sm ${tone === "danger" ? "border-red-200 bg-red-50 hover:bg-red-100/60" : "border-stone-200 bg-white hover:border-stone-300 hover:bg-stone-50"}`}
  >
    <span className="flex min-w-0 items-center gap-3">
      <span
        className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${tone === "danger" ? "bg-red-100 text-red-700" : "bg-[#FFF4E2] text-[#FF5F57]"}`}
      >
        <i className={`las ${icon} text-2xl`} />
      </span>
      <span className="min-w-0">
        <span
          className={`block text-sm font-bold ${tone === "danger" ? "text-red-700" : "text-stone-950"}`}
        >
          {title}
        </span>
        <span
          className={`mt-0.5 block text-xs leading-5 ${tone === "danger" ? "text-red-700/75" : "text-stone-500"}`}
        >
          {body}
        </span>
      </span>
...
const Profile: React.FC = () => {
  ...
  const activeListings = useMemo(
    () => listings.filter((l) => l.active && l.expiresAt > Date.now()),
    [listings],
  );
  ...
  useEffect(() => {
    if (!isOwnProfile) setActiveTab("active");
  }, [isOwnProfile]);
  useEffect(() => {
    if (isOwnProfile && location.hash === "#settings") setActiveTab("settings");
  ...
  useEffect(
    () => () => {
      if (avatarCrop?.src.startsWith("blob:"))
  ...
  const syncUserDisplayData = async (updates: {
    displayName?: string;
    photoURL?: string;
    location?: string;
  }) => {
    ...
    await Promise.all(
      (conversationSnap?.docs || []).map((item) =>
        updateDoc(doc(db, "conversations", item.id), {
          ...(updates.displayName
            ? { [`participantNames.${currentUser.uid}`]: updates.displayName }
            : {}),
          ...(updates.photoURL !== undefined
            ? { [`participantPhotos.${currentUser.uid}`]: updates.photoURL }
            : {}),
        }),
      ),
    );
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const snap = await getDoc(doc(db, "users", targetUserId)).catch(
        () => null,
      );
      ...
      const lSnap = await getDocs(
        query(collection(db, "listings"), where("userId", "==", targetUserId)),
      );
      const ls: Listing[] = [];
      ...
      if (isOwnProfile && p?.bookmarks?.length) {
        ...
        setBookmarkedListings(
          bookmarked.filter((l) => l.active && l.expiresAt > Date.now()),
        );
      } else setBookmarkedListings([]);

      const rSnap = await getDocs(
        query(collection(db, "ratings"), where("toUserId", "==", targetUserId)),
      ).catch(() => null);
      const rs: Rating[] = [];
  ...
  const reauthenticate = async (password: string) => {
    if (!currentUser?.email)
      throw new Error("Your account does not have an email address attached.");
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      await syncUserDisplayData({
        displayName: cleanName,
        location: editLocation,
      });
      setProfile((current) =>
        current ? ({ ...current, ...updates } as UserProfile) : null,
      );
      setListings((current) =>
        current.map((listing) => ({ ...listing, userName: cleanName })),
      );
  ...
  const handlePhotoUpload = async (
    ...
    if (!file.type.startsWith("image/"))
      return setSaveError("Please upload an image file.");
    if (file.size > 5 * 1024 * 1024)
      return setSaveError("Profile photo must be under 5MB.");
  ...
  const handleAvatarPointerMove = (
    ...
    const nextX =
      avatarDragState.cropX + (event.clientX - avatarDragState.startX) / 2.4;
    const nextY =
      avatarDragState.cropY + (event.clientY - avatarDragState.startY) / 2.4;
    setAvatarCrop((current) =>
      current
        ? { ...current, x: clamp(nextX, -100, 100), y: clamp(nextY, -100, 100) }
        : current,
    );
  };
  ...
  const resetAvatarCrop = () =>
    setAvatarCrop((current) =>
      current ? { ...current, zoom: 1, x: 0, y: 0 } : current,
    );
  ...
  const uploadCroppedProfilePhoto = async () => {
    ...
    const previousPhotoURL =
      profile?.photoURL || userProfile?.photoURL || currentUser.photoURL || "";
    try {
      const cropped = await cropProfilePhoto(
        avatarCrop.src,
        avatarCrop.zoom,
        avatarCrop.x,
        avatarCrop.y,
      ...
      const photoRef = ref(
        storage,
        `users/${currentUser.uid}/profile-photo-${Date.now()}.webp`,
      );
      await uploadBytes(photoRef, cropped, { contentType: "image/webp" });
      const photoURL = await getDownloadURL(photoRef);
      await setDoc(
        doc(db, "users", currentUser.uid),
        { photoURL, lastSeen: Date.now() },
        { merge: true },
      );
      await updateProfile(currentUser, { photoURL }).catch(() => undefined);
      await syncUserDisplayData({ photoURL });
      setProfile((current) =>
        current ? { ...current, photoURL, lastSeen: Date.now() } : current,
      );
      setListings((current) =>
        current.map((listing) => ({ ...listing, userPhoto: photoURL })),
      );
      setBookmarkedListings((current) =>
        current.map((listing) =>
          listing.userId === currentUser.uid
            ? { ...listing, userPhoto: photoURL }
            : listing,
        ),
      );
      await refreshProfile();
      ...
      setSaveError(err?.message || "Profile photo failed to upload.");
    } finally {
  ...
  const handleChangeEmail = async () => {
    ...
    if (!cleanEmail || cleanEmail === currentUser.email)
      return setSaveError("Enter a new email address first.");
    ...
    try {
      ...
      setSaveMessage(`Confirmation email sent to ${cleanEmail}.`);
    } catch (err: any) {
      setSaveError(
        err?.code === "auth/wrong-password" ||
          err?.code === "auth/invalid-credential"
          ? "Current password is incorrect."
          : err?.message || "Email change failed.",
      );
    } finally {
  ...
  const handleChangePassword = async () => {
    ...
    if (newPassword.length < 6)
      return setSaveError("New password must be at least 6 characters.");
    if (newPassword !== confirmNewPassword)
      return setSaveError("New passwords do not match.");
    setAccountLoading(true);
    setSaveError("");
    setSaveMessage("");
    try {
    ...
    try {
      ...
      await Promise.all([
        ...listingSnap.docs.map((item) =>
          updateDoc(doc(db, "listings", item.id), {
            active: false,
            deactivatedOwner: true,
          }),
        ),
        ...(conversationSnap?.docs || []).map((item) =>
          updateDoc(doc(db, "conversations", item.id), {
            [`participantNames.${currentUser.uid}`]: "Deleted account",
            [`participantPhotos.${currentUser.uid}`]: "",
          }),
        ),
        setDoc(
          doc(db, "users", currentUser.uid),
  ...
  const handleRenewListing = async (listing: Listing) => {
    ...
    setSaveMessage("");
    try {
      const settingsSnap = await getDoc(doc(db, "platform", "settings")).catch(
        () => null,
      );
      const days =
        Number(
          settingsSnap?.exists()
            ? settingsSnap.data().listingDays
            : DEFAULT_RENEW_DAYS,
        ) || DEFAULT_RENEW_DAYS;
      const expiresAt =
        Date.now() + Math.min(Math.max(days, 1), 45) * 24 * 60 * 60 * 1000;
      ...
      setListings((current) =>
        current.map((item) =>
          item.id === listing.id ? { ...item, active: true, expiresAt } : item,
        ),
      );
    ...
    try {
      ...
      const convRef = await addDoc(collection(db, "conversations"), {
        ...
        participantNames: {
          ...
          [targetUserId]: recipientName,
        },
        participantPhotos: {
          [currentUser.uid]:
            userProfile?.photoURL || currentUser.photoURL || "",
          [targetUserId]: profile.photoURL || "",
        },
        listingId: "",
  ...
  const removeBookmark = async (listingId: string) => {
    ...
    await setDoc(
      doc(db, "users", currentUser.uid),
      { bookmarks: arrayRemove(listingId), lastSeen: Date.now() },
      { merge: true },
    );
    window.dispatchEvent(
      new CustomEvent("reshelved:bookmark-updated", {
        detail: { listingId, bookmarked: false },
      }),
    );
    setBookmarkedListings((current) =>
  ...
  const tabs = isOwnProfile
    ? [
        {
          id: "active" as ProfileTab,
          label: "Active listings",
          icon: "la-book-open",
          count: activeListings.length,
        },
        {
          id: "expired" as ProfileTab,
  ...
  const renderAvatarCropModal = () => {
    ...
    return (
      <ListingImageCropModal
        ...
        dragState={avatarDragState}
        onDragStart={handleAvatarPointerDown}
        onDragMove={handleAvatarPointerMove}
        onDragEnd={() => setAvatarDragState(null)}
        onZoomChange={(zoom) =>
          setAvatarCrop((current) => (current ? { ...current, zoom } : current))
        }
        onReset={resetAvatarCrop}
        onSkip={closeAvatarCrop}
        onUsePhoto={uploadCroppedProfilePhoto}
      />
    );
  };
  ...
  const renderSecurityModal = () => {
    ...
    const subtitle =
      securityModal === "delete"
        ? "This action deactivates your listings and deletes your login account."
        : "Confirm the change securely using your current password.";
    return (
        ...
        <div className="w-full max-w-md rounded-[28px] bg-white p-6 shadow-2xl">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              ...
              className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-stone-100 text-stone-600 hover:bg-stone-200"
          ...
          {securityModal === "email" && (
            <div className="mt-5 space-y-3">
              ...
              <button
                ...
                className={`w-full cursor-pointer ${primaryButtonClass}`}
              >
                {accountLoading ? "Sending..." : "Send confirmation email"}
              </button>
            </div>
          )}
          {securityModal === "password" && (
      ...
      <div className="mx-auto max-w-[1180px] px-4 py-8 pb-10 sm:px-6 sm:pb-20">
        <div className="animate-pulse rounded-[32px] border border-stone-200 bg-white p-6">
          <div className="flex items-center gap-5">
            <div className="h-24 w-24 rounded-[28px] bg-stone-200" />
  ...
  const renderListingGrid = (
    ...
    items.length === 0 ? (
      ...
      <div
        className={
          compact
            ? "grid grid-cols-1 gap-x-4 gap-y-5 sm:grid-cols-2 lg:grid-cols-2"
    ...
    <div className="mx-auto max-w-[1180px] px-4 py-8 pb-10 sm:px-6 sm:pb-20">
      ...
      <div className="mb-5">
        {cameFromListing ? (
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="inline-flex cursor-pointer items-center text-sm font-bold text-[#1665CC] hover:text-[#1254a9]"
          >
            ← Back to listing
          </button>
        ) : (
      ...
      <div className="grid gap-8 lg:grid-cols-[320px_minmax(0,1fr)] lg:items-start">
        <aside className="lg:sticky lg:top-24">
          <div className="rounded-[32px] border border-stone-200 bg-white p-6 shadow-sm">
            ...
            <div className="flex flex-col items-center text-center">
              <div className="relative">
                {profile.photoURL ? (
                  <img
                    ...
                    className="h-28 w-28 rounded-full object-cover ring-1 ring-stone-200"
                  />
                ) : (
                  <div className="flex h-28 w-28 items-center justify-center rounded-full bg-stone-100 text-4xl font-bold text-stone-500 ring-1 ring-stone-200">
                    {profile.displayName?.[0]?.toUpperCase() || "U"}
                  </div>
                )}
                {isOwnProfile && (
                  <label className="absolute -bottom-2 left-1/2 flex -translate-x-1/2 cursor-pointer items-center gap-1 rounded-full border border-stone-200 bg-white px-3 py-1.5 text-xs font-bold text-stone-700 shadow-sm hover:bg-stone-50">
                    <i className="las la-camera text-base" />
              ...
              <h1 className="mt-7 text-2xl font-bold tracking-tight text-stone-950">
                {profile.displayName}
              </h1>
              <div className="mt-3 flex flex-wrap justify-center gap-2 text-xs font-semibold text-stone-600">
                {profile.location && (
                  <span className="rounded-full border border-stone-200 px-3 py-1.5">
                ...
                {ratings.length > 0 && (
                  <span className="rounded-full border border-stone-200 px-3 py-1.5">
                    <i className="las la-star mr-1 text-[#F7AF31]" />
              ...
              {profile.bio ? (
                ...
                <p className="mt-5 text-sm leading-6 text-stone-500">
                  Add a short bio in Settings so buyers know who they are
                  dealing with.
                </p>
              ) : null}
              {!isOwnProfile && currentUser && (
                <button
                  ...
                  className="mt-6 inline-flex w-full cursor-pointer items-center justify-center gap-2 rounded-full bg-[#FF5F57] px-5 py-3 text-sm font-bold text-white hover:bg-[#e84f48] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <i className="las la-comment text-lg" />
                ...
                {tabs.map((tab) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex h-11 cursor-pointer items-center justify-between rounded-full px-4 text-left text-[16px] transition ${selected ? "border border-stone-300 bg-gradient-to-b from-stone-100 to-stone-200 font-semibold text-black" : "font-medium text-stone-700 hover:bg-stone-100"}`}
                    >
                      <span className="flex min-w-0 items-center gap-3.5">
                        <i className={`las ${tab.icon} text-xl`} />
                        {tab.label}
                      </span>
                      {tab.count !== null && (
                        <span
                          className={`rounded-full px-2 py-0.5 text-xs ${selected ? "bg-white text-stone-700" : "bg-stone-100 text-stone-500"}`}
                        >
                          {tab.count}
                        </span>
                      )}
        ...
        <main className="min-w-0">
          {!isOwnProfile && (
            <section>
              <SectionHeader
                ...
                subtitle={`${activeListings.length} active book${activeListings.length === 1 ? "" : "s"} listed on Reshelved.`}
              />
              {renderListingGrid(
                activeListings.slice(0, 2),
                <EmptyState
                  icon="la-book-open"
                  title="No active listings"
                  body="This user does not have any books available right now."
                />,
                true,
              )}
            </section>
          ...
          {isOwnProfile && activeTab === "active" && (
            <section>
              <SectionHeader
                ...
                subtitle="Books buyers can currently discover and message you about."
                action={
                  <Link to="/create" className={primaryButtonClass}>
              ...
              {renderListingGrid(
                ...
                <EmptyState
                  icon="la-book-open"
                  title="No active listings"
                  body="List your first book so people nearby can find it."
                  action={
                    <Link to="/create" className={primaryButtonClass}>
          ...
          {isOwnProfile && activeTab === "expired" && (
            <section>
              <SectionHeader
                eyebrow="Shelf history"
                title="Expired listings"
                subtitle="Renew books you still want to keep visible."
              />
              {expiredListings.length === 0 ? (
                <EmptyState
                  icon="la-book-open"
                  title="No expired listings"
                  body="Expired books will appear here when their listing period ends."
                />
              ) : (
                <div className="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2 lg:grid-cols-3">
                  {expiredListings.map((l) => (
                    <div key={l.id}>
                      ...
                      <button
                        type="button"
                        onClick={() => handleRenewListing(l)}
                        disabled={renewingId === l.id}
                        className="mt-3 w-full cursor-pointer rounded-full bg-[#1665CC] px-4 py-3 text-sm font-bold text-white hover:bg-[#1254a9] disabled:cursor-not-allowed disabled:opacity-60"
                      >
                        {renewingId === l.id ? "Renewing..." : "Renew listing"}
                      </button>
                    </div>
                  ))}
                </div>
            ...
            <section>
              <SectionHeader
                ...
                action={
                  <Link
                    to="/browse"
                    className="text-sm font-bold text-[#1665CC] hover:text-[#1254a9]"
                  >
                    Browse books
                  </Link>
                }
              ...
              {bookmarkedListings.length === 0 ? (
                <EmptyState
                  icon="la-heart"
                  title="No bookmarked books yet"
                  body="Save books while browsing and they will appear here."
                  action={
                    <Link to="/browse" className={primaryButtonClass}>
                      Find books to save
                    </Link>
                  }
                />
                  ...
                  {bookmarkedListings.map((l) => (
                    <div key={l.id}>
                      ...
                      <button
                        ...
                        className="mt-3 w-full cursor-pointer rounded-full border border-stone-200 px-4 py-3 text-sm font-bold text-stone-700 hover:bg-stone-50"
                      >
                        Remove bookmark
                      </button>
                    </div>
          ...
          {isOwnProfile && activeTab === "profile" && (
            <section>
              <SectionHeader
                ...
                subtitle="This is the information other users use to decide whether to contact you."
              />
              <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
                <div className="rounded-[28px] border border-stone-200 bg-white p-6">
                  <h3 className="text-lg font-bold text-stone-950">Details</h3>
                ...
                <div className="rounded-[28px] border border-stone-200 bg-white p-6">
                  <h3 className="text-lg font-bold text-stone-950">Reviews</h3>
                  {ratings.length === 0 ? (
                      ...
                      {ratings.map((r) => (
                        ...
                        >
                          ...
                          <p className="mt-2 text-xs text-stone-400">
                            {new Date(r.createdAt).toLocaleDateString()} · Re:{" "}
                            {r.listingTitle}
                          </p>
                        </div>
                      ))}
          ...
          {isOwnProfile && activeTab === "settings" && (
            <section>
              <SectionHeader
                ...
                subtitle="Update profile details, email, password, and account status."
              />
              <div className="grid gap-6 xl:grid-cols-2">
                ...
                <div className="rounded-[28px] border border-stone-200 bg-white p-6">
                  <h3 className="text-lg font-bold text-stone-950">Security</h3>
                  <p className="mt-1 text-sm leading-6 text-stone-500">
                    Choose an action to open a focused editor.
                  </p>
                  <div className="mt-5 space-y-3">
                    <SecurityCard
                      icon="la-envelope"
                      title="Change email"
                      body="Send a confirmation link to a new email address."
                      onClick={() => setSecurityModal("email")}
                    />
                    <SecurityCard
                      icon="la-lock"
                      title="Change password"
                      body="Update your password after confirming the current one."
                      onClick={() => setSecurityModal("password")}
                    />
                    <SecurityCard
                      icon="la-trash-alt"
                      title="Delete account"
                      body="Deactivate listings and remove your login account."
                      tone="danger"
