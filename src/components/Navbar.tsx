      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {currentUser && (
              <>
                <Link to="/messages" className={`relative px-3 py-2 rounded-lg transition font-medium text-sm ${messageUnreadCount > 0 ? 'text-primary-700 bg-primary-50' : 'text-stone-600 hover:text-primary-700 hover:bg-primary-50'}`}>
                  Messages
                  <MessageBadge />
                </Link>
              ...
              <>
                ...
                <div className="relative">
                  <button onClick={() => setMenuOpen(!menuOpen)} className="cursor-pointer flex items-center gap-2 px-3 py-1.5 rounded-full hover:bg-stone-100 transition">
                    {profilePhoto ? (
                      <img src={profilePhoto} alt={profileName} className="w-8 h-8 rounded-full object-cover" />
                    ) : (
                      <div className="w-8 h-8 rounded-full bg-stone-200 text-stone-500 flex items-center justify-center font-semibold text-sm">{profileName?.[0]?.toUpperCase() || 'U'}</div>
                    )}
          ...
          <div className="md:hidden flex items-center gap-2">
            ...
            <button className="cursor-pointer p-2 rounded-lg hover:bg-stone-100" onClick={() => setMobileOpen(!mobileOpen)}>
