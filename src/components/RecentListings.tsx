...
const RecentListings: React.FC<{ excludeId?: string; limit?: number }> = ({ excludeId, limit = 4 }) => {
  ...
  return <section className="mt-12"><h2 className="text-xl font-bold text-stone-900 mb-5">You may also like</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">{listings.map((listing) => <BookCard key={listing.id} listing={listing} />)}</div></section>;
};
